...
import org.hyperledger.besu.savm.code.OpcodeInfo;
...
/** Represents SAVM code associated with an account. */
public class Code {
  ...
  public String prettyPrint() {
    ...
    ps.println("0x # Legacy SAVM Code");
    while (i < len) {
   ...
   * SAVM's JumpService to set the valid jump destinations for the code.
   *
  ...
  /**
   * Computes a bitmask where each bit set to 1 indicates a valid `JUMPDEST` opcode in the SAVM
  ...
  long[] calculateJumpDestBitMask() {
    ...
    // Get the raw SAVM bytecode as a byte array (no copying)
