...
public class MessageCallProcessor extends AbstractMessageProcessor {
  ...
  public MessageCallProcessor(
    ...
    this(savm, precompiles, forceCommitAddresses, TransferLogEmitter.NOOP);
  }
   ...
   * @param precompiles the precompiles
   */
  public MessageCallProcessor(final SAVM savm, final PrecompileContractRegistry precompiles) {
    this(savm, precompiles, Collections.emptySet(), TransferLogEmitter.NOOP);
  }
  ...
  private void transferValue(final MessageFrame frame) {
    ...
    // Emit transfer log for nonzero value transfers (no-op before SilaAmsterdam, SIP-7708 after)
