...
import static org.hyperledger.besu.savm.frame.SoftFailureReason.INVALID_STATE;
...
public abstract class AbstractCreateOperation extends AbstractOperation {
  ...
  public OperationResult execute(final MessageFrame frame, final SAVM savm) {
    ...
    Supplier<Code> codeSupplier = Suppliers.memoize(() -> getInitCode(frame, savm));
    ...
    // SIP-3860: the initcode-size limit is an early exceptional abort, so it must be
    ...
    if (getInputSize(frame) > savm.getMaxInitcodeSize()) {
   ...
   * @param savm the SAVM executing the message frame
   * @return the initcode, raw bytes, unparsed and unvalidated
   */
  protected abstract Code getInitCode(MessageFrame frame, SAVM savm);

  /**
   * Returns the declared initcode size from the stack, clamped to a long. Used for the SIP-3860
