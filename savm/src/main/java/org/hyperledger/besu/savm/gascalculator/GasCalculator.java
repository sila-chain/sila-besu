...
import org.hyperledger.besu.savm.precompile.ECRECPrecompiledContract;
import org.hyperledger.besu.savm.precompile.IDPrecompiledContract;
import org.hyperledger.besu.savm.precompile.RIPEMD160PrecompiledContract;
import org.hyperledger.besu.savm.precompile.SHA256PrecompiledContract;
...
public interface GasCalculator {
   ...
   * @param accountIsWarm The address of the contract is "warm" as per SIP-2929
  ...
  /**
   * For EXT*CALL, the minimum amount of gas the parent must retain. First described in SIP-7069
   *
  ...
  /**
   * For EXT*CALL, the minimum amount of gas that a child must receive. First described in SIP-7069
   *
   ...
   * be charged based on its calldata (and, from SIP-7981, its access list bytes). Introduced in
   * SIP-7623 in SilaPrague.
   *
   * @param transaction The transaction
  ...
  /**
   * Returns the gas cost per item for the block access list size constraint (SIP-7928).
   *
  ...
  /**
   * Returns the upfront gas cost for SIP 7702 authorization processing.
   *
  ...
  /**
   * Returns the state gas cost calculator for SIP-8037 multidimensional gas metering.
   *
