 ...
 */
package org.hyperledger.besu.savm.toy;
...
public class SavmToyCommand implements Runnable {
  ...
  public void run() {
    ...
    SilaMainnetPrecompiledContracts.populateForIstanbul(
        precompileContractRegistry, savm.getGasCalculator());
    ...
    do {
      ...
      while (!messageFrameStack.isEmpty()) {
        ...
        if (lastLoop && messageFrameStack.isEmpty()) {
          final long savmGas = gas - messageFrame.getRemainingGas();
          out.println();
          out.printf(
              "{ \"gasUser\": \"0x%s\", \"timens\": %d, \"time\": %d }%n",
              Long.toHexString(savmGas), lastTime, lastTime / 1000);
        }
      }
      lastTime = stopwatch.elapsed().toNanos();
      stopwatch.reset();
