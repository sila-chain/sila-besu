...
import static org.hyperledger.besu.savm.frame.MessageFrame.Type.MESSAGE_CALL;
...
class CodeTest {
  ...
  void shouldReuseJumpDestMap() {
    final JumpOperation operation = new JumpOperation(savm.getGasCalculator());
    ...
    OperationResult result = operation.execute(frame, savm);
    ...
    result = operation.execute(frame, savm);
    assertNull(result.getHaltReason());
    Mockito.verify(getsCached, times(1)).calculateJumpDestBitMask();
  }
