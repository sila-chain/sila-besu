...
- Sunsetting features is now complete - for more context on the reasoning behind these removals, read [this blog post](https://www.lfdecentralizedtrust.org/blog/sunsetting-tessera-and-simplifying-hyperledger-besu)
  - Proof of Work consensus (PoW) removal is now complete:
    - Remove PoW mining infrastructure: PoW mining coordinator, executor, block creator/miner, nonce generators, PoW solver, and PoWObserver are deleted. `nonceGenerator` is removed from `MiningConfiguration`. SilaMainnet genesis files with `silash` config can no longer mine PoW blocks. [#10656](https://github.com/sila-chain/sila-besu/issues/10656)
    - Remove Silash and PoW validation code: SilHash algorithm, PoWHasher, ProofOfWorkValidationRule, CalculatedDifficultyValidationRule, EpochCalculator, and DirectAcyclicGraphSeed are deleted. `powHasher` is removed from `ProtocolSpec`/`ProtocolSpecBuilder`. [#10659](https://github.com/sila-chain/sila-besu/issues/10659)
...
- Fix `debug_trace*` `storage` field to emit only for SLOAD/SSTORE opcodes showing the single slot touched, matching the execution-apis spec and gsil behaviour [#10176](https://github.com/sila-chain/sila-besu/pull/10176)
- Fix blob transaction sidecar validation: detect missing sidecars, KZG commitment/versioned-hash mismatches, and type/size mismatches against sil/68 announcements; disconnect offending peers immediately [#10510](https://github.com/hyperledger/besu/pull/10510)
...
- `savmtool blocktest --verbose` flag, default off, removes noise from output [#10348](https://github.com/sila-chain/sila-besu/pull/10348)
...
### Breaking Changes
- Clique consensus has been removed. Besu can no longer start or mine on pure Clique networks. Syncing networks that started as Clique and have since transitioned to PoS via `terminalTotalDifficulty` (e.g. Linea SilaMainnet) are still supported. [#9852](https://github.com/hyperledger/besu/pull/9852)
...
### Bug fixes
- Fix sil/69 snap sync receipt root mismatch by correctly identifying Frontier transaction type in SyncTransactionReceiptEncoder [#9900](https://github.com/hyperledger/besu/pull/9900)
...
- **Chain pruning CLI options have been redesigned with new behavior:** [#9637](https://github.com/hyperledger/besu/pull/9637)
  ...
  - sil_simulateV1 returns BLOCK_NOT_FOUND instead of null success response when called with a future block [#9837](https://github.com/hyperledger/besu/pull/9837)
  - sil_simulateV1: when validation is enabled, map UPFRONT_COST_EXCEEDS_BALANCE (-38014) error code to -32602 [#9837](https://github.com/hyperledger/besu/pull/9837)

### Upcoming Breaking Changes
- RPC changes to enhance compatibility with other ELs
  - Block number parameter in RPCs will only support hex values. Support for non-hex (decimal) block number parameters is deprecated.
  - This affects several RPCs, including `admin_logsRemoveCache`, `debug_getRawHeader`, `sil_call`, `sil_simulateV1`, `trace_call` and more.
- SilaHolesky network is deprecated [#9437](https://github.com/hyperledger/besu/pull/9437)
- Sunsetting features - for more context on the reasoning behind the deprecation of these features, including alternative options, read [this blog post](https://www.lfdecentralizedtrust.org/blog/sunsetting-tessera-and-simplifying-hyperledger-besu)
    - Proof of Work consensus (PoW)
...
### Additions and Improvements
- Support substring and glob matching for `--test-name` in `block-test` savmtool subcommand [#9790](https://github.com/hyperledger/besu/pull/9790)
...
#### Performance
- SAVM optimisations - Improves 70% of EEST benchmarks [#9775](https://github.com/hyperledger/besu/pull/9775)
- SAVM optimisations - Improve SAR, SHR and SHL opcodes performance [#9796](https://github.com/hyperledger/besu/pull/9796)
...
- Fix QBFT SilaShanghai support by reintroducing NotApplicableWithdrawals withdrawals validator [#9830](https://github.com/hyperledger/besu/pull/9830)
...
- RPC changes to enhance compatibility with other ELs
  - RPCs using filter parameter including `sil_getLogs` and `trace_filter` return an error if `fromBlock` is greater than `toBlock`, or if `toBlock` extends beyond chain head (previously returned an empty list) [#9604](https://github.com/hyperledger/besu/pull/9604)
...
- Performance
  ...
  - Optimise SIP-196 AltBn128: EcAdd 33-128% faster, EcMul 8% faster [#9570](https://github.com/hyperledger/besu/pull/9570)
...
- Add SIP-7778 to SilaAmsterdam [#9664](https://github.com/hyperledger/besu/pull/9664)
...
- RPC changes to enhance compatibility with other ELs
  ...
  - sil_createAccessList now returns success result if execution reverted [#9358](https://github.com/hyperledger/besu/pull/9358)
  - Return null result if block not found for `debug_accountAt`, `debug_setHead`, `sil_call`, `sil_getBlockReceipts`, `sil_getProof`, `sil_simulateV1`, `sil_getBalance`, `sil_getCode`, `sil_getStorageAt`, `sil_getTransactionCount` [#9303](https://github.com/hyperledger/besu/pull/9303)
- Remove PoW specific RPCs: `sil_getMinerDataByBlockHash`, `sil_getMinerDataByBlockNumber`, `miner_setCoinbase`, `miner_setSilerbase` [#9481](https://github.com/hyperledger/besu/pull/9481), `sil_coinbase` [#9487](https://github.com/hyperledger/besu/pull/9487)
...
- Add Linea named networks for `linea_sila-mainnet` and `linea_sepolia` [#9436](https://github.com/hyperledger/besu/pull/9436), [#9518](https://github.com/hyperledger/besu/pull/9518)
- Add `sil_subscribe` and `sil_unsubscribe` support to IPC service [#9504](https://github.com/hyperledger/besu/pull/9504)
...
- Fix an issue where Besu does not support `0x80` as transaction type when decoding sil/69 receipts [#9520](https://github.com/hyperledger/besu/issues/9520)
...
### Additions and Improvements
- Add SilaOsaka, BPO1 and BPO2 fork times for sila-mainnet [#9380](https://github.com/hyperledger/besu/pull/9380)
- Add blockTimestamp to receipt logs for `sil_getBlockReceipts` and `sil_getTransactionReceipt` results [#9294](https://github.com/hyperledger/besu/pull/9294)
...
- Increase sila-mainnet gas limit to 60M [#9339](https://github.com/hyperledger/besu/pull/9339)
...
### Additions and Improvements
- Add blockTimestamp to `sil_getLogs` result [#9278](https://github.com/hyperledger/besu/pull/9278)
- Add `--silstats-report-interval` CLI option to control silstats reporting frequency instead of using hardcoded 5-second interval [#9271](https://github.com/hyperledger/besu/pull/9271)
...
This RC is still pending burn in for sila-mainnet.
...
Only upgrade to this release if you're experiencing the sil_subscribe or silstats known issue from 25.9.0.
...
### Bug fixes
- Fix sil_subscribe RPC failing returning a block response [#9212](https://github.com/hyperledger/besu/pull/9212)
- Fix silstats integration failing to provide block updates to silstats server [#9220](https://github.com/hyperledger/besu/pull/9220)
...
### Known issue 
Affects users of sil_subscribe (WebSocket) eg SSV, and silstats integration
...
- Add SilaOsaka, BPO1 and BPO2 fork times for holesky, hoodi and sepolia [#9196](https://github.com/hyperledger/besu/pull/9196)
...
- Remove support for Sila protocol version `sil/67`. [#9008](https://github.com/hyperledger/besu/pull/9008). 
...
- Improve performance of OperandStack resizes for deep stacks (> 100 elements). Impacts general SAVM performance while working with deep stacks [#8869](https://github.com/hyperledger/besu/pull/8869)
...
- LUKSO SilaMainnet Pectra Hardfork [#9070](https://github.com/hyperledger/besu/pull/9070)
...
- Add `--attempt-cache-bust` to savmtool benchmark subcommand [#8985](https://github.com/hyperledger/besu/pull/8985)
- Add gas usage metric to sil_call [#9019](https://github.com/hyperledger/besu/pull/9019)
- Improve P256Verify precompile performance [#9035](https://github.com/hyperledger/besu/pull/9035)

#### Fusaka devnets
- SIP-7910 - `sil_config` JSON-RPC Method [#8417](https://github.com/hyperledger/besu/pull/8417), [#8946](https://github.com/hyperledger/besu/pull/8946), [#9015](https://github.com/hyperledger/besu/pull/9015)

### Bug fixes
- Fix bug with `sil_estimateGas` on QBFT - use zero address when doing simulation against `pending` block [#9031](https://github.com/hyperledger/besu/pull/9031)
...
- Introduce history expiry behaviour for sila-mainnet [8875](https://github.com/hyperledger/besu/pull/8875)
...
- Implement rewardPercentile cap in sil_feeHistory [#8748](https://github.com/hyperledger/besu/pull/8748)
...
- Use sil/69 by default [#8858](https://github.com/hyperledger/besu/pull/8858)
...
### Performance
- Increase sila-mainnet gas limit to 45M [#8824](https://github.com/hyperledger/besu/pull/8824)
...
- SIP-7825 - Transaction gas limit cap [#8700](https://github.com/hyperledger/besu/pull/8700)
- SIP-7823 - Modexp upper bounds [#8632](https://github.com/hyperledger/besu/pull/8632)
- SIP-7892 - Max number of blobs per transaction [#8761](https://github.com/hyperledger/besu/pull/8761)
- SIP-7934 - RLP Execution Block Size Limit [#8765](https://github.com/hyperledger/besu/pull/8765)
- SIP-7951 - Precompile for secp256r1 Curve Support [#8750](https://github.com/hyperledger/besu/pull/8750)
...
| org.hyperledger.besu.internal:api | org.hyperledger.besu.internal:besu-silereum-api |
| org.hyperledger.besu.internal:blockcreation | org.hyperledger.besu.internal:besu-silereum-blockcreation |
| org.hyperledger.besu.internal:core | org.hyperledger.besu.internal:besu-silereum-core |
| org.hyperledger.besu.internal:sil | org.hyperledger.besu.internal:besu-silereum-sil |
| org.hyperledger.besu.internal:p2p | org.hyperledger.besu.internal:besu-silereum-p2p |
| org.hyperledger.besu.internal:permissioning | org.hyperledger.besu.internal:besu-silereum-permissioning |
| org.hyperledger.besu.internal:referencetests | org.hyperledger.besu.internal:besu-silereum-referencetests |
| org.hyperledger.besu.internal:rlp | org.hyperledger.besu.internal:besu-silereum-rlp |
| org.hyperledger.besu.internal:trie | org.hyperledger.besu.internal:besu-silereum-trie |
...
### Additions and Improvements
- Add sil/69 protocol for optional use by using the `--Xsil-capability-max=69` flag (currently defaults to 68) [#8519](https://github.com/hyperledger/besu/pull/8519)
...
- SAVM skip unnecessary state access for CALL/LOG [#8639](https://github.com/hyperledger/besu/pull/8639)
- SIP-5920 - PAY opcode [#8498](https://github.com/hyperledger/besu/pull/8498)
- SIP-7892 - Blob Parameter Only (BPO) forks [#8671](https://github.com/hyperledger/besu/pull/8671)
- SIP-7883 - MODEXP gas cost increase [#8707](https://github.com/hyperledger/besu/pull/8707)
...
## 25.5.0
### Breaking Changes
- Changes to gas estimation algorithm for `sil_estimateGas` and `sil_createAccessList` [#8478](https://github.com/hyperledger/besu/pull/8478) - if you require the previous behavior, specify `--estimate-gas-tolerance-ratio=0.0`
...
### Additions and Improvements
- SilaSepolia checkpoint block has been updated to the merge block [#8584](https://github.com/hyperledger/besu/pull/8584)
...
- Refine gas estimation algorithm for `sil_estimateGas` and `sil_createAccessList` [#8478](https://github.com/hyperledger/besu/pull/8478) including a new option to specify `--estimate-gas-tolerance-ratio`
- Increase default target-gas-limit to 60M for SilaHolesky and SilaSepolia [#8594](https://github.com/hyperledger/besu/pull/8594)
...
### Breaking Changes
- `--compatibility-sil64-forkid-enabled` has been removed. [#8541](https://github.com/hyperledger/besu/pull/8541)
...
### Additions and Improvements
- Add support for `sil_simulateV1`. [#8406](https://github.com/hyperledger/besu/pull/8406)
- New metric `besu_peers_peer_count_by_client` to report the count of peers by client name [#8515](https://github.com/hyperledger/besu/pull/8515)- Add SilaPrague fork time for sila-mainnet. [#8521](https://github.com/hyperledger/besu/pull/8521)
- Add SilaPrague fork time for sila-mainnet. [#8521](https://github.com/hyperledger/besu/pull/8521)
...
- Transaction indexing will be disabled by default in a future release for snap sync and checkpoint sync modes. This will break RPCs that use transaction hash for historical queries.
- Support for block creation on networks running a pre-Byzantium fork is deprecated for removal in a future release, after that in order to update Besu on nodes that build blocks, your network needs to be upgraded at least to the Byzantium fork. The main reason is to simplify world state management during block creation, since before Byzantium for each selected transaction, the receipt must contain the root hash of the modified world state, and this does not play well with the new plugin features and future work on parallelism.
- `--compatibility-sil64-forkid-enabled` has been deprecated and will be removed in the next release. [#8492](https://github.com/hyperledger/besu/pull/8492)
...
- Removed support for Sila protocol versions `sil/62`, `sil/63`, `sil/64`, and `sil/65`. [#8492](https://github.com/hyperledger/besu/pull/8492)
...
- Performance: Consensys/tuweni 2.7.0 reduces boxing/unboxing overhead on some SAVM opcodes, like PushX and Caller [#8330](https://github.com/hyperledger/besu/pull/8330), [#8461](https://github.com/hyperledger/besu/pull/8461)

### Bug fixes
- Fix QBFT and IBFT transitions that change the mining beneficiary [#8387](https://github.com/hyperledger/besu/issues/8387)
- `sil_getLogs` - empty topic is a wildcard match [#8420](https://github.com/hyperledger/besu/pull/8420)
...
### Breaking Changes
NOTE: This release breaks native Windows compatibility for sila-mainnet sila configurations.  As the prague(pectra) hardfork require 
BLS12-381 precompiles and besu does not currently have a pure java implementation of bls12-381, only platforms which
have support in besu-native can run sila-mainnet sila configurations.  Windows support via WSL should still continue to work.
...
- SilaMainnet sila now REQUIRES native crypto libraries, so only linux and macos(darwin) are supported sila-mainnet configurations [#8418](https://github.com/hyperledger/besu/pull/8418)
...
### Additions and Improvements
#### SilaPrague
- Increase sila-mainnet and SilaSepolia gas limit to 36M [#8249](https://github.com/hyperledger/besu/pull/8249)
- Update SilaHolesky and SilaSepolia deposit contract addresses [#8346](https://github.com/hyperledger/besu/pull/8346)
#### Plugins
...
- Update `sil_getLogs` to return a `Block not found` error when the requested block is not found. [#8290](https://github.com/hyperledger/besu/pull/8290)
- Change `Invalid block, unable to parse RLP` RPC error message to `Invalid block param (block not found)` [#8328](https://github.com/hyperledger/besu/pull/8328)
...
- Upgrade to execution-spec-tests v4.1.0 including better SIP-2537 coverage for BLS [#8402](https://github.com/hyperledger/besu/pull/8402)
...
## 25.2.2 hotfix
- Pectra - SilaSepolia: Fix for deposit contract log decoding [#8383](https://github.com/hyperledger/besu/pull/8383)

## 25.2.1 hotfix
- Pectra - update SilaHolesky and SilaSepolia deposit contract addresses [#8346](https://github.com/hyperledger/besu/pull/8346)
...
#### SilaPrague
- Add timestamps to enable SilaPrague hardfork on SilaSepolia and SilaHolesky test networks [#8163](https://github.com/hyperledger/besu/pull/8163)
- Update system call addresses to match [devnet-6](https://github.com/sila-chain/execution-spec-tests/releases/) values [#8209](https://github.com/hyperledger/besu/issues/8209) 
...
- Change the output format of debug_trace* and trace_* calls to match Gsil behaviour
...
- Implement SIP-7840: Add blob schedule to config files [#8042](https://github.com/hyperledger/besu/pull/8042)
- Allow gasPrice (legacy) and 1559 gasPrice params to be specified simultaneously for `sil_call`, `sil_createAccessList`, and `sil_estimateGas` [#8059](https://github.com/hyperledger/besu/pull/8059)
- Improve debug_traceBlock calls performance and reduce output size [#8103](https://github.com/hyperledger/besu/pull/8103)
- Add support for SIP-7702 transaction in the txpool [#8018](https://github.com/hyperledger/besu/pull/8018) [#7984](https://github.com/hyperledger/besu/pull/7984)
- Add support for `movePrecompileToAddress` in `StateOverrides` (`sil_call`)[8115](https://github.com/hyperledger/besu/pull/8115)
- Default target-gas-limit to 36M for holesky [#8125](https://github.com/hyperledger/besu/pull/8125)
- Add SIP-7623 - Increase calldata cost [#8093](https://github.com/hyperledger/besu/pull/8093)
...
### Breaking Changes
- Removed Retestsil rpc service and commands [#7833](https://github.com/hyperledger/besu/pull/7783)
...
- With the upgrade of the Promsileus Java Metrics library, there are the following changes:
  ...
  - The `_created` timestamps are not returned by default, you can set the env var `BESU_OPTS="-Dio.promsileus.exporter.includeCreatedTimestamps=true"` to enable them
...
- Add account and state overrides to `sil_call` [#7801](https://github.com/hyperledger/besu/pull/7801) and `sil_estimateGas` [#7890](https://github.com/hyperledger/besu/pull/7890)
- Add RPC WS options to specify password file for keystore and truststore [#7970](https://github.com/hyperledger/besu/pull/7970)
- Promsileus Java Metrics library upgraded to version 1.3.3 [#7880](https://github.com/hyperledger/besu/pull/7880)
- Add histogram to Promsileus metrics system [#7944](https://github.com/hyperledger/besu/pull/7944)
- Improve newPayload and FCU logs [#7961](https://github.com/hyperledger/besu/pull/7961)
- Proper support for `pending` block tag when calling `sil_estimateGas` and `sil_createAccessList` [#7951](https://github.com/hyperledger/besu/pull/7951)
...
- Correct default parameters for frontier transactions in `sil_call` and `sil_estimateGas` [#7965](https://github.com/hyperledger/besu/pull/7965)
- Correctly parse nonce as hex in `sil_call` account overrides [#7999](https://github.com/hyperledger/besu/pull/7999)
...
- LUKSO SilaCancun Hardfork [#7686](https://github.com/hyperledger/besu/pull/7686)
...
- Align gas cap calculation for transaction simulation to Gsil approach [#7703](https://github.com/hyperledger/besu/pull/7703)
...
- Fix sil_feeHistory rewards when bounded by configuration [#7750](https://github.com/hyperledger/besu/pull/7750)
...
- Include current chain head block when computing `sil_maxPriorityFeePerGas` [#7485](https://github.com/hyperledger/besu/pull/7485)
...
- Fix for `sil_gasPrice` could not retrieve block error [#7482](https://github.com/hyperledger/besu/pull/7482)
...
- `savmtool` was not respecting the `--genesis` setting, resulting in unexpected trace results. [#7433](https://github.com/hyperledger/besu/pull/7433)
...
- Honor block number or tag parameter in sil_estimateGas and sil_createAccessList [#7502](https://github.com/hyperledger/besu/pull/7502)
...
- Wrap WorldUpdater into SAVMWorldupdater [#7434](https://github.com/hyperledger/besu/pull/7434)
- Bump besu-native to 0.9.4 [#7456](https://github.com/hyperledger/besu/pull/7456)=

### Bug fixes
- Correct entrypoint in Docker savmtool [#7430](https://github.com/hyperledger/besu/pull/7430)
...
- Added `block-test` subcommand to the savmtool which runs blockchain reference tests [#7293](https://github.com/hyperledger/besu/pull/7293)
- removed PKI backed QBFT [#7310](https://github.com/hyperledger/besu/pull/7310)
- Implement gnark-crypto for sip-2537 [#7316](https://github.com/hyperledger/besu/pull/7316)
- Improve blob size transaction selector [#7312](https://github.com/hyperledger/besu/pull/7312)
- Added SIP-7702 [#7237](https://github.com/hyperledger/besu/pull/7237)
- Implement gnark-crypto for sip-196 [#7262](https://github.com/hyperledger/besu/pull/7262)
...
### Bug fixes
- Fix `sil_call` deserialization to correctly ignore unknown fields in the transaction object. [#7323](https://github.com/hyperledger/besu/pull/7323)
...
### Additions and Improvements
- Support for sil_maxPriorityFeePerGas [#5658](https://github.com/hyperledger/besu/issues/5658)
...
- A full and up to date implementation of EOF for SilaPrague [#7169](https://github.com/hyperledger/besu/pull/7169)
...
- `admin_nodeInfo` JSON/RPC call returns the currently active SAVM version [#7127](https://github.com/hyperledger/besu/pull/7127)
- Improve the selection of the most profitable built block [#7174](https://github.com/hyperledger/besu/pull/7174)

### Bug fixes
- Make `sil_gasPrice` aware of the base fee market [#7102](https://github.com/hyperledger/besu/pull/7102)
...
### Known Issues
- [Frequency: occasional < 10%] Chain download halt. Only affects new syncs (new nodes syncing from scratch). Symptom: Block import halts, despite having a full set of peers and world state downloading finishing. Generally restarting besu will resolve the issue. We are tracking this in [#6884](https://github.com/hyperledger/besu/pull/6884)
- [Frequency: occasional < 10%] Low peer numbers. More likely to occur on testnets (holesky and sepolia) but also can occur on sila-mainnet. Symptom: peer count stays at 0 for an hour or more. Generally restarting besu will resolve the issue. We are tracking this in [#6805](https://github.com/hyperledger/besu/pull/6805)
...
- `sil_call` for blob tx allows for empty `maxFeePerBlobGas` [#6731](https://github.com/hyperledger/besu/pull/6731)
...
- Fix SilaShanghai/QBFT block import bug when syncing new nodes [#6765](https://github.com/hyperledger/besu/pull/6765)
...
- Requesting the Sila Node Record (ENR) to acquire the fork id from bonded peers is now enabled by default, so the following change has been made [#5628](https://github.com/hyperledger/besu/pull/5628):
...
### Additions and Improvements
- Upgrade Promsileus and Opentelemetry dependencies [#6422](https://github.com/hyperledger/besu/pull/6422)
...
- Added configuration options for `pragueTime` to genesis file for SilaPrague fork development [#6473](https://github.com/hyperledger/besu/pull/6473)
...
- Support for `shanghaiTime` fork and SilaShanghai SAVM smart contracts in QBFT/IBFT chains [#6353](https://github.com/hyperledger/besu/pull/6353)
...
- New `sil_blobBaseFee`JSON-RPC method [#6581](https://github.com/hyperledger/besu/pull/6581)
- Add blob transaction support to `sil_call` [#6661](https://github.com/hyperledger/besu/pull/6661)
- Add blobs to `sil_feeHistory` [#6679](https://github.com/hyperledger/besu/pull/6679)
...
- Add `sila_min_gas_price` and `sila_min_priority_fee` metrics to track runtime values of `min-gas-price` and `min-priority-fee` [#6587](https://github.com/hyperledger/besu/pull/6587)
...
### Additions and Improvements
- Adds timestamp to enable SilaCancun upgrade on sila-mainnet [#6545](https://github.com/hyperledger/besu/pull/6545)
...
### Bug fixes
- INTERNAL_ERROR from `sil_estimateGas` JSON/RPC calls [#6344](https://github.com/hyperledger/besu/issues/6344)
...
- Fluent SAVM API definition for Tangerine Whistle had incorrect code size validation configured [#6382](https://github.com/hyperledger/besu/pull/6382)
...
- Set Sila Classic sila-mainnet activation block for Spiral network upgrade [#6267](https://github.com/hyperledger/besu/pull/6267)
