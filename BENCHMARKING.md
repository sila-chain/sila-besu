...
JMH tasks are defined in various modules such as `sila:core`, `sila:sil`, and `sila:rlp`.

### ▶️  Run All Benchmarks

```bash
./gradlew :sila:core:jmh
```
...
```bash
./gradlew :sila:core:jmh --rerun-tasks
```
...
```bash
./gradlew :sila:core:jmh -Pincludes=Mod -Pexcludes=Mul,Add,SMod -Pcases=MOD_256_128,MOD_256_192 --rerun-tasks
```
...
### engine_getPayloadBodies parallelization (`sila:api`)
...
```bash
./gradlew :sila:api:jmh -Pincludes=EngineGetPayloadBodiesParallel --rerun-tasks --no-daemon
```
