...
public interface SilStatsConnectOptions {
  ...
  /**
   * Gets the silstats report interval.
   *
   * @return the silstats report interval
   */
  Integer getSilStatsReportInterval();

  /**
   * Creates an SilStatsConnectOptions from the given parameters with default report interval.
   *
   * @param url the url of the connection
   * @param contact the contact of the connection
   * @param caCert the CA certificate of the connection
   * @return the SilStatsConnectOptions
   */
  static SilStatsConnectOptions fromParams(
      final String url, final String contact, final Path caCert) {
    return fromParams(url, contact, caCert, 5); // Default to 5 seconds
  }

  /**
   * Creates an SilStatsConnectOptions from the given parameters.
   *
   * @param url the url of the connection
   * @param contact the contact of the connection
   * @param caCert the CA certificate of the connection
   * @param silStatsReportInterval the silstats report interval
   * @return the SilStatsConnectOptions
   */
  static SilStatsConnectOptions fromParams(
      final String url,
      ...
      final Path caCert,
      final Integer silStatsReportInterval) {
    try {
      ...
      return ImmutableSilStatsConnectOptions.builder()
          .scheme(scheme)
          .nodeName(nodeName)
          .secret(secret)
          .host(uri.getHost())
          ...
          .contact(contact)
          .caCert(caCert)
          .silStatsReportInterval(silStatsReportInterval)
          .build();
