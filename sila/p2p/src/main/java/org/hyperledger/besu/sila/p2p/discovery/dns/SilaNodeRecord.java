...
public record SilaNodeRecord(
  ...
  public SilaNodeRecord {
    if (ip.isEmpty() && ipV6.isEmpty()) {
      throw new IllegalArgumentException(
          "Unable to create SilaNodeRecord with no IPv4 or IPv6 address");
    }
  }

  /**
   * Creates an SilaNodeRecord from an ENR string
   *
   * @param enr the ENR string
   * @return the SilaNodeRecord
   */
  public static SilaNodeRecord fromEnr(final String enr) {
    NodeRecord nodeRecord = NodeRecordFactory.DEFAULT.fromEnr(enr);
    return fromNodeRecord(nodeRecord);
  }

  /**
   * Creates an SilaNodeRecord from a NodeRecord
   *
   * @param nodeRecord the NodeRecord
   * @return the SilaNodeRecord
   */
  public static SilaNodeRecord fromNodeRecord(final NodeRecord nodeRecord) {
    final Map<String, Object> fields = new HashMap<>();
    nodeRecord.forEachField(fields::put);
    return new SilaNodeRecord(
  ...
  public boolean equals(final Object obj) {
    ...
    final SilaNodeRecord other = (SilaNodeRecord) obj;
