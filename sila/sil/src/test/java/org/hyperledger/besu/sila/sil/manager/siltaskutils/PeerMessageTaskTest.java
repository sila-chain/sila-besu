...
/**
 * Tests silTasks that interact with a single peer to retrieve data from the network.
 *
...
public abstract class PeerMessageTaskTest<T>
  ...
  public void completesWhenPeerReturnsPartialResult() {
    ...
    final RespondingSilPeer.Responder responder =
        RespondingSilPeer.partialResponder(
            blockchain,
    ...
    final RespondingSilPeer respondingSilPeer =
        SilProtocolManagerTestUtil.createPeer(silProtocolManager, 32);

    // Execute task and wait for response
    final AtomicReference<T> actualResult = new AtomicReference<>();
    final AtomicReference<SilPeer> actualPeer = new AtomicReference<>();
    final AtomicBoolean done = new AtomicBoolean(false);
    final T requestedData = generateDataToBeRequested();
    final SilTask<AbstractPeerTask.PeerTaskResult<T>> task = createTask(requestedData);
    ...
    assertThat(actualPeer.get()).isEqualTo(respondingSilPeer.getSilPeer());
  }
  ...
  public void failsWhenNoPeersAreAvailable() {
    ...
    final SilTaskException silException = (SilTaskException) error;
    assertThat(silException.reason()).isEqualTo(SilTaskException.FailureReason.NO_AVAILABLE_PEERS);
  ...
  public void recordsTimeoutAgainstPeerWhenTaskTimesOut() {
    ...
    assertThat(future.isCompletedExceptionally()).isTrue();
    assertThat(
            respondingSilPeer.getSilPeer().timeoutCounts().values().stream()
  ...
  protected SilPeer createPeer() {
    final PeerConnection peerConnection = new MockPeerConnection(Set.of(SilProtocol.LATEST));
