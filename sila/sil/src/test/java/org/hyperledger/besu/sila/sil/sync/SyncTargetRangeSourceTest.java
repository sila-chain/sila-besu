...
public class SyncTargetRangeSourceTest {
  ...
  private final SilScheduler silScheduler = mock(SilScheduler.class);

  @BeforeEach
  @SuppressWarnings("unchecked")
  void setUp() {
    when(silScheduler.scheduleFutureTask(any(Supplier.class), any(Duration.class)))
  ...
  public void shouldDelayBeforeRetryingRequestForCheckpointHeadersAfterEmptyResponse() {
    ...
    assertThat(source.next()).isNull();
    verify(rangeHeaders).getNextRangeHeaders(peer, commonAncestor);
    verify(silScheduler).scheduleFutureTask(any(Supplier.class), eq(RETRY_DELAY_DURATION));
  }
