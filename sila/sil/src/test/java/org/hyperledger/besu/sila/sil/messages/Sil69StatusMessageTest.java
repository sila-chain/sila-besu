...
public class Sil69StatusMessageTest {

  private final int version = SilProtocol.SIL69.getVersion();
  ...
  public void shouldNotHaveTotalDifficultWhen69FromRawInput() {
    ...
    forkId.writeTo(out);
    out.endList();
    // The decoder must surface this as an RLPException so the sil protocol handler's
  ...
  public void shouldRejectSil68LayoutFromProdBesuV26Dot2Develop73d07f9() {
    ...
    final RLPException ex =
        assertThrows(
            ...
            "Real-world malformed Status from a broken sil/69 encoder must be rejected");
    assertThat(ex.getMessage()).contains("version must be <= 68");
  }
}
