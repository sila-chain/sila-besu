  ...
  public void setUp() {
    ...
    silProtocolManager =
        SilProtocolManagerTestBuilder.builder()
            ...
            .build();
    silContext = spy(silProtocolManager.silContext());

    final SilScheduler silScheduler = spy(silContext.getScheduler());
    syncTaskCapture = ArgumentCaptor.forClass(Runnable.class);
    doNothing().when(silScheduler).scheduleSyncWorkerTask(syncTaskCapture.capture());
    doReturn(silScheduler).when(silContext).getScheduler();
