...
class NewPooledTransactionHashesMessageProcessorTest {
  ...
  public void setup() {
    ...
    when(transactionPoolConfiguration.getUnstable().getSil65TrxAnnouncedBufferingPeriod())
  ...
  void shouldAddNewHashesToRequestQueue() {
    ...
    messageHandler.processNewPooledTransactionHashesMessage(
        peer1,
        NewPooledTransactionHashesMessage.create(transactionList, SilProtocol.LATEST),
        now(),
        ofMinutes(1));
  ...
  void shouldScheduleGetPooledTransactionsTaskWhenNewTransactionAddedFromPeerForTheFirstTime() {
    messageHandler.processNewPooledTransactionHashesMessage(
        ...
        NewPooledTransactionHashesMessage.create(
            List.of(transaction1, transaction2), SilProtocol.LATEST),
        now(),
        ofMinutes(1));
  ...
  void shouldNotScheduleGetPooledTransactionsTaskTwice() {
    when(silScheduler.scheduleFutureTaskWithFixedDelay(
            any(FetcherCreatorTask.class), any(Duration.class), any(Duration.class)))
    ...
    messageHandler.processNewPooledTransactionHashesMessage(
        peer1,
        NewPooledTransactionHashesMessage.create(
            Collections.singletonList(transaction1), SilProtocol.LATEST),
        now(),
        ofMinutes(1));
    ...
    messageHandler.processNewPooledTransactionHashesMessage(
        peer1,
        NewPooledTransactionHashesMessage.create(
            Collections.singletonList(transaction2), SilProtocol.LATEST),
        now(),
        ofMinutes(1));
