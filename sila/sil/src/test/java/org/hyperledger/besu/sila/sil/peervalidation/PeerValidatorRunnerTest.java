...
public class PeerValidatorRunnerTest {
  ...
  public void checkPeer_schedulesFutureCheckWhenPeerNotReady() {
    ...
    final PeerValidatorRunner runner =
        spy(new PeerValidatorRunner(silProtocolManager.silContext(), validator));
    runner.checkPeer(peer);

    assertThat(peer.isFullyValidated()).isFalse();

    verify(runner, times(1)).checkPeer(eq(peer));
    verify(validator, never()).validatePeer(eq(silProtocolManager.silContext()), eq(peer));
  ...
  public void checkPeer_handlesInvalidPeer() {
    ...
    when(validator.validatePeer(eq(silProtocolManager.silContext()), eq(peer)))
    ...
    verify(validator, times(1)).validatePeer(eq(silProtocolManager.silContext()), eq(peer));
