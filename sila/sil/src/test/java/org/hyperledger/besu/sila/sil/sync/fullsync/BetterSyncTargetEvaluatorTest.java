...
public class BetterSyncTargetEvaluatorTest {
  ...
  private final SilPeer currentTarget = peer(CURRENT_TARGET_HEIGHT, CURRENT_TARGET_TD);
  ...
  public void setupMocks() {
    when(silPeers.getBestPeerComparator()).thenReturn(SilPeers.TOTAL_DIFFICULTY_THEN_HEIGHT);
  }

  @Test
  public void shouldNotSwitchTargetsIfNoBestPeerIsAvailable() {
    when(silPeers.bestPeer()).thenReturn(Optional.empty());
  ...
  private void bestPeerWithDelta(final long height, final long totalDifficulty) {
    final SilPeer bestPeer =
        peer(CURRENT_TARGET_HEIGHT + height, CURRENT_TARGET_TD + totalDifficulty);
    when(silPeers.bestPeer()).thenReturn(Optional.of(bestPeer));
  }

  private SilPeer peer(final long chainHeight, final long totalDifficulty) {
