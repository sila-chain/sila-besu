...
public abstract class AbstractMessageTaskTest<T, R> {
  ...
  protected abstract SilTask<R> createTask(T requestedData);

  protected abstract void assertResultMatchesExpectation(
      T requestedData, R response, SilPeer respondingPeer);

  @Test
  public void completesWhenPeersAreResponsive() {
    // Setup a responsive peer
    final RespondingSilPeer.Responder responder = getFullResponder();
    ...
    final SilTask<R> task = createTask(requestedData);
    ...
    assertResultMatchesExpectation(requestedData, actualResult.get(), respondingPeer.getSilPeer());
    assertNoBadBlocks();
  }
  ...
  protected RespondingSilPeer.Responder getFullResponder() {
    return RespondingSilPeer.blockchainResponder(
