...
public class AbstractSilTaskTest {
  ...
  public void shouldCancelAllIncompleteSubtasksWhenMultipleIncomplete() {
    ...
    final SilTaskWithMultipleSubtasks task =
        new SilTaskWithMultipleSubtasks(Arrays.asList(subtask1, subtask2));
  ...
  public void shouldAnyCancelIncompleteSubtasksWhenMultiple() {
    ...
    final SilTaskWithMultipleSubtasks task =
        new SilTaskWithMultipleSubtasks(Arrays.asList(subtask1, subtask2, subtask3));
  ...
  public void shouldIgnoreMultipleRuns() {
    ...
    final AbstractSilTask<Void> incrementingTask =
        new AbstractSilTask<>(new NoOpMetricsSystem()) {
  ...
  public void shouldInvokeTimingMethods() {
    final AbstractSilTask<Void> task =
        new AbstractSilTask<Void>(mockOperationTimer) {
          @Override
          protected void executeTask() {}
        };
  ...
  public void shouldHaveSpecificMetricsLabels() {
    ...
    final String[] lastLabelNames = {"AbstractSilTask"};
    ...
    new AbstractSilTask<>(instrumentedLabeler) {
      @Override
      protected void executeTask() {
        // no-op
      }
    };
    assertThat(lastLabelNames[0]).isNotEqualTo("AbstractSilTask");
  }

  private class SilTaskWithMultipleSubtasks extends AbstractSilTask<Void> {

    private final List<CompletableFuture<?>> subtasks;

    private SilTaskWithMultipleSubtasks(final List<CompletableFuture<?>> subtasks) {
