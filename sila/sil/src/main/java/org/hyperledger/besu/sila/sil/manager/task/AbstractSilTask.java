...
public abstract class AbstractSilTask<T> implements SilTask<T> {
  ...
  private static OperationTimer buildOperationTimer(
    ...
    if (silTasksTimer == NoOpMetricsSystem.NO_OP_LABELLED_1_OPERATION_TIMER) {
      ...
      return silTasksTimer.labels(taskName);
    }
  }
  ...
  /**
   * Utility for executing completable futures that handles cleanup if this SilTask is cancelled.
   *
  ...
  /**
   * Helper method for sending subTask to worker that will clean up if this SilTask is cancelled.
   *
  ...
  protected final <S> CompletableFuture<S> executeWorkerSubTask(
      final SilScheduler scheduler, final Supplier<CompletableFuture<S>> subTask) {
