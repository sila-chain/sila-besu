...
public class MergePeerFilter implements MergeStateHandler, UnverifiedForkchoiceListener {
  ...
  public boolean disconnectIfPoW(final StatusMessage status, final SilPeer peer) {
  ...
  public boolean disconnectIfGossipingBlocks(final Message message, final SilPeer peer) {
    final int code = message.getData().getCode();
    if (isFinalized()
        && (code == SilProtocolMessages.NEW_BLOCK
            || code == SilProtocolMessages.NEW_BLOCK_HASHES)) {
