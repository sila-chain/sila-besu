...
/**
 * Represents a task to be executed on an SilPeer by the PeerTaskExecutor
 *
...
public interface PeerTask<T> {
  ...
  /**
   * Gets the request data to send to the SilPeer
   *
   * @return the request data to send to the SilPeer
   */
  MessageData getRequestMessage(final Set<Capability> agreedCapabilities);

  /**
   * Parses and processes the MessageData response from the SilPeer
   *
  ...
  /**
   * Gets a Predicate that checks if an SilPeerImmutableAttributes is suitable for this PeerTask
   *
   * @return a Predicate that checks if an SilPeerImmutableAttributes is suitable for this PeerTask
   */
  Predicate<SilPeerImmutableAttributes> getPeerRequirementFilter();
