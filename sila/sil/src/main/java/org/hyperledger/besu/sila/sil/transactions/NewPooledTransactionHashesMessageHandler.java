...
class NewPooledTransactionHashesMessageHandler implements SilMessages.MessageCallback {
  ...
  @Override
  public void exec(final SilMessage message) {
    if (isEnabled.get()) {
      final Capability capability = message.getPeer().getConnection().capability(SilProtocol.NAME);
