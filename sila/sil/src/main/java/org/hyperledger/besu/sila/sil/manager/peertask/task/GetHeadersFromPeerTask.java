...
public class GetHeadersFromPeerTask implements PeerTask<List<BlockHeader>> {
  ...
  public Predicate<SilPeerImmutableAttributes> getPeerRequirementFilter() {
    return (silPeer) -> {
      return isPoS || silPeer.estimatedChainHeight() >= blockNumber;
    };
  }
  ...
  public void postProcessResult(final PeerTaskExecutorResult<List<BlockHeader>> result) {
    if (!result.silPeers().isEmpty() && result.result().isPresent()) {
      result.result().get().stream()
          .max(Comparator.comparingLong(ProcessableBlockHeader::getNumber))
          .ifPresent(
              (highestBlockHeader) ->
                  result.silPeers().getLast().chainState().update(highestBlockHeader));
    }
  }
