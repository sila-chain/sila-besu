...
public final class StatusMessage extends AbstractMessageData {
  ...
  private static StatusMessage create(final SilStatus status) {
  ...
  @Override
  public int getCode() {
    return SilProtocolMessages.STATUS;
  }

  /**
   * Return The sil protocol version the associated node is running.
   *
   * @return The sil protocol version the associated node is running.
   */
  ...
  /**
   * Return The block range of the associated node's local blockchain. (Sil/69)
   *
  ...
  private SilStatus status() {
    if (status == null) {
      ...
      status = SilStatus.readFrom(input);
    }
    return status;
  }
  ...
  private static class SilStatus {
    ...
    SilStatus(
      ...
      checkArgument(
          blockRange == null || protocolVersion >= SilProtocolVersion.V69,
          "blockRange is only supported for protocol version >= 69");
      checkArgument(
          blockRange != null || protocolVersion <= SilProtocolVersion.V68,
          "blockRange must be present for protocol version >= 69");
      checkArgument(
          totalDifficulty == null || protocolVersion <= SilProtocolVersion.V68,
          "totalDifficulty must be not present for protocol version >= 69");
      checkArgument(
          totalDifficulty != null || protocolVersion >= SilProtocolVersion.V69,
    ...
    public void writeTo(final RLPOutput out) {
      /*
       * (sil/68): `[version, networkId, td, blockHash, genesis, forkId]`
       * (sil/69): `[version, networkId, genesis, forkId, earliestBlock, latestBlock, blockHash]`
       */
      out.startList();
    ...
    public static SilStatus readFrom(final RLPInput in) {
      final Hash bestHash;
      final Hash genesisHash;
      final ForkId forkId;
