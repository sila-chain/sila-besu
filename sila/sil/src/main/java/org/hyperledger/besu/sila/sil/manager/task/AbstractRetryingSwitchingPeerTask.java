...
public abstract class AbstractRetryingSwitchingPeerTask<T> extends AbstractRetryingPeerTask<T> {
  ...
  private final Set<SilPeer> triedPeers = new HashSet<>();
  private final Set<SilPeer> failedPeers = new HashSet<>();
  ...
  protected AbstractRetryingSwitchingPeerTask(
    ...
    super(silContext, maxRetries, isEmptyResponse, metricsSystem);
  }
  ...
  protected abstract CompletableFuture<T> executeTaskOnCurrentPeer(final SilPeer peer);

  @Override
  protected CompletableFuture<T> executePeerTask(final Optional<SilPeer> assignedPeer) {

    final Optional<SilPeer> maybePeer =
        assignedPeer
    ...
    final SilPeer peerToUse = maybePeer.get();
  ...
  private Optional<SilPeer> selectNextPeer() {
    final Optional<SilPeer> maybeNextPeer = nextPeerToTry();
  ...
  private void refreshPeers() {
    ...
    if (peers.peerCount() >= peers.getMaxPeers()) {
      failedPeers.stream()
          ...
          .min(SilPeers.MOST_USEFUL_PEER)
          .or(() -> peers.streamAvailablePeers().min(SilPeers.MOST_USEFUL_PEER))
          .ifPresent(
              peer -> {
                LOG.atDebug()
                    .setMessage(
                    ...
                    .log();
                peer.silPeer().disconnect(DisconnectReason.USELESS_PEER_BY_REPUTATION);
              });
    }
  }
}
