...
public class SilScheduler {
  private static final Logger LOG = LoggerFactory.getLogger(SilScheduler.class);
  ...
  public SilScheduler(
    ...
    this(
        MonitoredExecutors.newFixedThreadPool(
            SilScheduler.class.getSimpleName() + "-Workers", syncWorkerCount, metricsSystem),
        MonitoredExecutors.newScheduledThreadPool(
            SilScheduler.class.getSimpleName() + "-Timer", 1, metricsSystem),
        MonitoredExecutors.newBoundedThreadPool(
            SilScheduler.class.getSimpleName() + "-Transactions",
            txWorkerCount,
            txWorkerQueueSize,
            metricsSystem),
        MonitoredExecutors.newCachedThreadPool(
            SilScheduler.class.getSimpleName() + "-Services", metricsSystem),
        MonitoredExecutors.newFixedThreadPool(
            SilScheduler.class.getSimpleName() + "-Computation",
            computationWorkerCount,
            metricsSystem),
        MonitoredExecutors.newCachedThreadPool(
            SilScheduler.class.getSimpleName() + "-BlockCreation", metricsSystem));
  }

  protected SilScheduler(
  ...
  public <T> CompletableFuture<T> timeout(final SilTask<T> task, final Duration timeout) {
