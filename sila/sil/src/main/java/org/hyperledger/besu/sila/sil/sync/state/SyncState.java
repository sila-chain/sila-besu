...
public class SyncState implements NewPayloadListener {
  ...
  public void setSyncTarget(final SilPeer peer, final BlockHeader commonAncestor) {
  ...
  private Optional<ChainHeadEstimate> getSyncTargetChainHead() {
    return syncTarget.map(SyncTarget::peer).map(SilPeer::chainStateSnapshot);
  }

  public Optional<ChainHeadEstimate> getBestPeerChainHead() {
    return silPeers.bestPeerWithHeightEstimate().map(SilPeer::chainStateSnapshot);
  }
