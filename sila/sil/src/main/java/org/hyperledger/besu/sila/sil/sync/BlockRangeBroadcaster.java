...
public class BlockRangeBroadcaster {
  ...
  public BlockRangeBroadcaster(final SilContext silContext, final Blockchain blockchain) {
   ...
   */
  @VisibleForTesting
  void handleBlockRangeUpdateMessage(final SilMessage message) {
    try {
  ...
  private void handleInvalidMessage(
      final SilMessage message, final DisconnectMessage.DisconnectReason reason) {
  ...
  void broadcastBlockRange(
    ...
    List<SilPeer> peers = getPeersSupportingBlockRangeUpdate();
    LOG.debug(
   ...
   */
  private void broadcastToPeer(final SilPeer peer, final BlockRangeUpdateMessage message) {
