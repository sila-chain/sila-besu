...
public abstract class WorldDownloadState<REQUEST extends TasksPriorityProvider> {
  ...
  private final Set<SilTask<?>> outstandingRequests =
  ...
  protected synchronized void cleanupQueues() {
    for (final SilTask<?> outstandingRequest : outstandingRequests) {
  ...
  public void addOutstandingTask(final SilTask<?> task) {
    outstandingRequests.add(task);
  }

  public void removeOutstandingTask(final SilTask<?> task) {
  ...
  public CompletableFuture<Void> startDownload(
      final WorldStateDownloadProcess worldStateDownloadProcess, final SilScheduler silScheduler) {
    this.worldStateDownloadProcess = worldStateDownloadProcess;
    final CompletableFuture<Void> processFuture = worldStateDownloadProcess.start(silScheduler);

    processFuture.whenComplete(
        (result, error) -> {
          if (error != null
              && !(ExceptionUtils.rootCause(error) instanceof CancellationException)) {
            // The pipeline is only ever cancelled by us or shutdown closing the SilScheduler
