...
public class SilMessages {
  ...
  public Optional<MessageData> dispatch(final SilMessage silMessage, final Capability capability) {
    final int code = silMessage.getData().getCode();

    // trigger arbitrary side-effecting listeners
    Optional.ofNullable(listenersByCode.get(code))
        .ifPresent(
            listeners -> listeners.forEach(messageCallback -> messageCallback.exec(silMessage)));
        ...
        .map(
            messageResponseConstructor ->
                messageResponseConstructor.response(
                    silMessage.getPeer(), silMessage.getData(), capability));
  }
  ...
  public interface MessageCallback {
    void exec(SilMessage message);
  }

  @FunctionalInterface
  public interface MessageResponseConstructor {
    MessageData response(SilPeer peer, MessageData message, Capability capability);
  }
}
