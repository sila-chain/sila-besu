...
public class TrailingPeerLimiter implements BlockAddedObserver {
  ...
  private static final Comparator<SilPeerImmutableAttributes> BY_CHAIN_HEIGHT =
  ...
  public void enforceTrailingPeerLimit() {
    ...
    final List<SilPeer> trailingPeers =
        silPeers
    ...
    while (!trailingPeers.isEmpty() && trailingPeers.size() > maxTrailingPeers) {
      final SilPeer peerToDisconnect = trailingPeers.remove(0);
      LOG.atDebug()
          .setMessage(
