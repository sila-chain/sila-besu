...
class BlockSizeTransactionSelectorTest {
  ...
  /**
   * Tests SIP-7778 gas accounting strategy. With SIP-7778, block gas is calculated using pre-refund
   ...
   */
  @Test
  void sip7778StrategyUsesPreRefundGasForBlockAccounting() {
    // Reconfigure with SIP-7778 strategy
    when(blockSelectionContext.protocolSpec().getBlockGasAccountingStrategy())
  ...
  /**
   * Verifies FRONTIER strategy uses post-refund gas (legacy behavior), contrasting with SIP-7778.
   */
  @Test
  ...
  /**
   * SIP-8037 2D gas: Post-processing correctly tracks gas state after successful transactions and
   * pre-processing rejects when a dimension is exhausted.
   */
  @Test
  void sip8037PostProcessingTracksGasAndPreProcessingRejectsWhenDimensionExhausted() {
    when(blockSelectionContext.pendingBlockHeader().getGasLimit()).thenReturn(30_000_000L);
    when(blockSelectionContext.protocolSpec().getBlockGasAccountingStrategy())
        .thenReturn(BlockGasAccountingStrategy.AMSTERDAM);
    selector = new BlockSizeTransactionSelector(blockSelectionContext, selectorsStateManager);
   ...
   */
  @Test
  void sip8037EffectiveGasUsedDrivesBlockFullCheck() {
  ...
  private PendingTransaction createPendingTransaction(final long gasLimit) {
    return PendingTransaction.newPendingTransaction(
        createTransaction(TransactionType.SIP1559, gasLimit), false, false, MAX_SCORE);
  }
