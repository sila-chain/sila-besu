...
import static org.hyperledger.besu.savmtool.T8nSubCommand.COMMAND_ALIAS;
...
/**
 * The T8nSubCommand class is responsible for executing an Sila State Test. It reads the initial
 * state, transactions, and environment from input files or stdin, executes the transactions in the
 * Sila Virtual Machine (SAVM), and writes the final state, transaction results, and traces to
 ...
 * to trace SAVM operations when the --json flag is specified.
 */
@Command(
    name = COMMAND_NAME,
...
public class T8nSubCommand implements Runnable {
   ...
   */
  @SuppressWarnings("unused")
  public T8nSubCommand(final SavmToolCommand parentCommand) {
  ...
  public void run() {
    ...
    try {
      ...
      initialWorldState = ReferenceTestWorldState.create(accounts, SavmConfiguration.DEFAULT);
    ...
    final T8nExecutor.T8nResult result =
        T8nExecutor.runTest(
            ...
            rejections,
            tracerManager,
            parentCommand.getSavmConfiguration());
