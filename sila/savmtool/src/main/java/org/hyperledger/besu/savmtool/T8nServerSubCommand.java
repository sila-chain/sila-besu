...
@CommandLine.Command(
    name = "t8n-server",
    description = "Run Sila State Test server",
...
public class T8nServerSubCommand implements Runnable {
  ...
  T8nServerSubCommand(final SavmToolCommand parentCommand) {
  ...
  void handleT8nRequest(
    ...
    try {
      ...
      try (ReferenceTestWorldState initialWorldState =
          ReferenceTestWorldState.create(accounts, SavmConfiguration.DEFAULT)) {
  ...
  private void sendHelp(final HttpServerRequest req, final ObjectMapper objectMapper) {
    ...
    for (var fork : SavmSpecVersion.values()) {
