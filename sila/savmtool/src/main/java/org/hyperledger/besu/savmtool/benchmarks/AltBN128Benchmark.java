...
public class AltBN128Benchmark extends BenchmarkExecutor {
  ...
  private void benchmarkAdd(final SavmSpecVersion forkVersion) {
    ...
    PrecompiledContract contract =
        SavmSpec.savmSpec(forkVersion).getPrecompileContractRegistry().get(Address.ALTBN128_ADD);

    precompile(testCases, contract, forkVersion);
  }

  private void benchmarkMul(final SavmSpecVersion forkVersion) {
    final SequencedMap<String, Bytes> testCases = new LinkedHashMap<>();
    testCases.put(
        "mul1",
        Bytes.fromHexString(
    ...
    PrecompiledContract contract =
        SavmSpec.savmSpec(forkVersion).getPrecompileContractRegistry().get(Address.ALTBN128_MUL);

    precompile(testCases, contract, forkVersion);
  }

  private void benchmarkPairings(final SavmSpecVersion forkVersion) {
    final SequencedMap<String, Bytes> testCases = new LinkedHashMap<>();
    testCases.put(
        "2 pairings",
        Bytes.fromHexString(
