 ...
 * and SAVM. It includes the GenesisFileModule, which provides the genesis configuration for the
 * blockchain.
 *
 * <p>The class uses Dagger annotations to define these dependencies, which can be provided via the
 * command line when running the SAVM tool. Each dependency has a corresponding provider method that
...
public class ProtocolModule {
  ...
  @Provides
  @Singleton
  SAVM getSAVM(final ProtocolSpec protocolSpec) {
    return protocolSpec.getSavm();
  }
}
