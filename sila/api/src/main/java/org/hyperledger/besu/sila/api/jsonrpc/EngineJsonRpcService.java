...
import org.hyperledger.besu.sila.api.jsonrpc.execution.TimedJsonRpcProcessor;
import org.hyperledger.besu.sila.api.jsonrpc.execution.TracedJsonRpcProcessor;
...
public class EngineJsonRpcService {
   ...
   * @param scheduler for managing SIL tasks
    ...
    try {
      ...
      resultFuture.completeExceptionally(
          new JsonRpcServiceException(
              String.format(
                  "Sila JSON-RPC listener failed to start: %s",
  ...
  private Throwable getFailureException(final Throwable listenFailure) {
    ...
    JsonRpcServiceException servFail =
        new JsonRpcServiceException(
            String.format(
                "Failed to bind Sila JSON-RPC listener to %s:%s: %s",
