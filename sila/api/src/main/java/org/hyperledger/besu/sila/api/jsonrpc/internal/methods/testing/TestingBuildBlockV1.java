...
public class TestingBuildBlockV1 implements JsonRpcMethod {
  ...
  private EngineGetPayloadResultV1 createVersionSpecificResponse(final BlockCreationResult result) {
    // min supported hard-fork is SilaPrague, since the response is expecting the executionRequests
    ...
    if (maybeBlockAccessList.isEmpty()) {
      // if no BAL then return SilaOsaka response
    ...
    // if BAL is present the return SilaAmsterdam response
