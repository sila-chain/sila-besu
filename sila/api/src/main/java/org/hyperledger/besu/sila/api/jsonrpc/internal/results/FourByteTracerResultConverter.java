...
/**
 * Converts Sila transaction traces into 4byte tracer format.
 *
 ...
 * remaining call data (excluding the 4-byte selector). This matches Gsil's 4byteTracer output
 * format.
 *
...
public final class FourByteTracerResultConverter {
  ...
  public static FourByteTracerResult convert(
    ...
    // Sort keys alphabetically to match Gsil's JSON encoding behavior
   ...
   *
   * <p>This matches Gsil's behavior where OnEnter fires for the transaction's entry into the
   ...
   * </ul>
   *
   * <p>This matches Gsil's OnEnter hook behavior which only fires when a call successfully enters a
   * new execution scope.
   *
  ...
  private static void processCall(
    ...
    // Check if call entered (depth increased) - matches Gsil's OnEnter behavior
   ...
   * Gsil's implementation where the size is calculated as `len(input) - 4`.
   *
