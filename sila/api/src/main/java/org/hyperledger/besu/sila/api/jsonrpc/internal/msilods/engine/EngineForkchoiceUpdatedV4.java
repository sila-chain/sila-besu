...
public class EngineForkchoiceUpdatedV4 extends AbstractEngineForkchoiceUpdatedV4 {
  ...
  protected Optional<JsonRpcErrorResponse> isPayloadAttributesValid(
    ...
    if (payloadAttributes.getParentBeaconBlockRoot() == null) {
      LOG.error(
          "Parent beacon block root hash not present in payload attributes after SilaAmsterdam hardfork");
    ...
    if (payloadAttributes.getSlotNumber() == null) {
      LOG.error("Slot number not present in payload attributes after SilaAmsterdam hardfork");
      return Optional.of(
          new JsonRpcErrorResponse(requestId, RpcErrorType.INVALID_SLOT_NUMBER_PARAMS));
    }

    if (payloadAttributes.getTargetGasLimit() == null) {
      LOG.error("Target gas limit not present in payload attributes after SilaAmsterdam hardfork");
      return Optional.of(
