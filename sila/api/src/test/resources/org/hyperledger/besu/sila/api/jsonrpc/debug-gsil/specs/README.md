...
from Gsil and are used to verify that alternative implementations (like Besu) produce matching output.

## Test Blockchain Source

These specifications were generated from a Gsil test node initialized with `../chain-data/blocks.bin`, which contains
 pre-built test blocks covering various SAVM scenarios. The Gsil node was started with debug APIs
...
gsil init --datadir ./data genesis.json

# Import pre-built blocks
gsil import --datadir ./data blocks.bin
```
...
1. **Validation**: Verify alternative implementations match Gsil's output
