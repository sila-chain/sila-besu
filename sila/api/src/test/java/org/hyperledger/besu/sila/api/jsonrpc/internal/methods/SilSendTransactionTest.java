...
public class SilSendTransactionTest {

  private JsonRpcMethod method;

  @BeforeEach
  public void before() {
    method = new SilSendTransaction();
  }

  @Test
  public void methodReturnExpectedName() {
    assertThat(method.getName()).isEqualTo("sil_sendTransaction");
  }
  ...
  public void requestIsMissingParameter() {
    final JsonRpcRequestContext request =
        new JsonRpcRequestContext(
            new JsonRpcRequest("2.0", "sil_sendTransaction", new String[] {}));

    final JsonRpcResponse expectedResponse =
        new JsonRpcErrorResponse(
            request.getRequest().getId(), RpcErrorType.SIL_SEND_TX_NOT_AVAILABLE);
