...
public class AdminJsonRpcHttpServiceTest extends JsonRpcHttpServiceTestBase {
  ...
  public void getPeers() throws Exception {
    ...
    caps.add(SilProtocol.LATEST);
    final List<SilPeer> peerList = new ArrayList<>();
    ...
    when(silPeersMock.streamAllPeers())
  ...
  private void assertPeerResultMatchesPeer(
      final JsonArray result, final Collection<SilPeer> peerList) {
    int i = -1;
    for (final SilPeer silPeer : peerList) {
      ...
      assertThat(silPeer.getConnection().getPeerInfo()).isEqualTo(jsonPeer);
    }
  }
