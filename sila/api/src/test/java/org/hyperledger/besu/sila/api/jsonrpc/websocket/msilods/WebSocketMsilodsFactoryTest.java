...
public class WebSocketMethodsFactoryTest {
  ...
  @BeforeEach
  public void before() {
    jsonRpcMethods.put("sil_unsubscribe", jsonRpcMethod());
    factory = new WebSocketMethodsFactory(subscriptionManager, jsonRpcMethods);
  }

  @Test
  public void websocketsFactoryShouldCreateSilSubscribe() {
    final JsonRpcMethod method = factory.methods().get("sil_subscribe");

    assertThat(method).isNotNull();
    assertThat(method).isInstanceOf(SilSubscribe.class);
  }

  @Test
  public void websocketsFactoryShouldCreateSilUnsubscribe() {
    final JsonRpcMethod method = factory.methods().get("sil_unsubscribe");

    assertThat(method).isNotNull();
    assertThat(method).isInstanceOf(SilUnsubscribe.class);
  }
