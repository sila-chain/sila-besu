...
public class EngineGetPayloadV2Test extends AbstractEngineGetPayloadTest {
  ...
  public void shouldReturnBlockForKnownPayloadId() {
    // should return withdrawals for a post-SilaShanghai block
  ...
  @Test
  public void shouldReturnExecutionPayloadWithoutWithdrawals_PreSilaShanghaiBlock() {
    final var resp = resp(RpcMethod.ENGINE_GET_PAYLOAD_V2.getMethodName(), mockPid);
    assertThat(resp).isInstanceOf(JsonRpcSuccessResponse.class);
    Optional.of(resp)
        .map(JsonRpcSuccessResponse.class::cast)
  ...
  public void shouldReturnUnsupportedForkIfBlockTimestampIsAfterSilaCancunMilestone() {
    // SilaCancun starts at timestamp 30
  ...
  protected long getValidPayloadTimestamp() {
    // V2 works with SilaShanghai but must be before SilaCancun (< 30)
    return 25L;
  }
}
