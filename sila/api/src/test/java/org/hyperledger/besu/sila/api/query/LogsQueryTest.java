...
public class LogsQueryTest {
  ...
  private static final LogTopic SRC20_TRANSFER_EVENT =
  ...
  public void testMatches() {
    final LogsQuery query =
        new LogsQuery(
            ...
            List.of(
                singletonList(SRC20_TRANSFER_EVENT),
    ...
    assertThat(query.matches(new Log(FIRST_ADDRESS, Bytes.EMPTY, List.of(SRC20_TRANSFER_EVENT))))
        .isFalse();
    assertThat(
            query.matches(
                new Log(
                    FIRST_ADDRESS,
                    Bytes.EMPTY,
                    List.of(SRC20_TRANSFER_EVENT, FIRST_ADDRESS_TOPIC))))
        .isTrue();
    assertThat(
            query.matches(
                new Log(
                    FIRST_ADDRESS,
                    Bytes.EMPTY,
                    List.of(SRC20_TRANSFER_EVENT, SECOND_ADDRESS_TOPIC))))
        .isTrue();
    assertThat(
            query.matches(
                new Log(
                    FIRST_ADDRESS,
                    Bytes.EMPTY,
                    List.of(SRC20_TRANSFER_EVENT, SECOND_ADDRESS_TOPIC, FIRST_ADDRESS_TOPIC))))
        .isTrue();
  }
