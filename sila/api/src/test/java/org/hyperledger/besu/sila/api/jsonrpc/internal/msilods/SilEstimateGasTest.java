...
public class SilEstimateGasTest {
  ...
  @Test
  public void shouldReturnErrorWhenTransientSip1559TransactionProcessorReturnsEmpty() {
    final JsonRpcRequestContext request = silEstimateGasRequest(sip1559TransactionCallParameter());
    when(transactionSimulator.process(
            eq(modifiedSip1559TransactionCallParameter()),
            eq(Optional.empty()),
            any(TransactionValidationParams.class),
            any(OperationTracer.class),
            eq(latestBlockHeader)))
  ...
  @Test
  public void
      shouldReturnGasEstimateWhenTransientSip1559TransactionProcessorReturnsResultSuccess() {
    final JsonRpcRequestContext request = silEstimateGasRequest(sip1559TransactionCallParameter());
    mockTransientProcessorResultGasEstimate(MIN_TX_GAS_COST, true, false, pendingBlockHeader);
  ...
  @Test
  public void
      shouldReturnGasEstimateErrorWhenTransientSip1559TransactionProcessorReturnsResultFailure() {
    final JsonRpcRequestContext request = silEstimateGasRequest(sip1559TransactionCallParameter());
    mockTransientProcessorResultGasEstimate(1L, false, false, latestBlockHeader);
  ...
  @Test
  public void shouldReturnErrorWhenSip1559TransactionProcessorReturnsTxInvalidReason() {
    final JsonRpcRequestContext request = silEstimateGasRequest(sip1559TransactionCallParameter());
    mockTransientProcessorResultTxInvalidReason(
        TransactionInvalidReason.UPFRONT_COST_EXCEEDS_BALANCE,
        "transaction up-front cost 10 exceeds transaction sender account balance 5",
  ...
  public void shouldUseTxGasLimitCapWhenLessThatBlockGasLimit() {
    ...
    verify(transactionSimulator)
        .processOnPending(
            eq(modifiedSip1559TransactionCallParameter(TX_GAS_LIMIT_CAP, OptionalLong.empty())),
            eq(Optional.empty()), // no account overrides
            eq(TransactionValidationParams.transactionSimulatorAllowUnderpricedAndFutureNonce()),
            any(OperationTracer.class),
            eq(pendingBlockHeader));
  ...
  private TransactionSimulatorResult getMockTransactionSimulatorResult(
    ...
    if (blockHeader == pendingBlockHeader) {
      ...
      when(transactionSimulator.processOnPending(
          ...
          .thenReturn(Optional.of(mockTxSimResult));
      when(transactionSimulator.processOnPending(
              eq(modifiedSip1559TransactionCallParameter(MIN_TX_GAS_COST, maybeNonce)),
              eq(Optional.empty()), // no account overrides
              any(TransactionValidationParams.class),
              any(OperationTracer.class),
              eq(blockHeader)))
          .thenReturn(Optional.of(mockTxSimResult));
      when(transactionSimulator.processOnPending(
              eq(modifiedSip1559TransactionCallParameter(BLOCK_GAS_LIMIT, maybeNonce)),
              eq(Optional.empty()), // no account overrides
              any(TransactionValidationParams.class),
              any(OperationTracer.class),
              eq(blockHeader)))
  ...
  private CallParameter modifiedSip1559TransactionCallParameter() {
    return modifiedSip1559TransactionCallParameter(MIN_TX_GAS_COST, OptionalLong.empty());
  }
