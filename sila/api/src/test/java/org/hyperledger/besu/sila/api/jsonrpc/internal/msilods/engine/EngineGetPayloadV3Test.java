...
public class EngineGetPayloadV3Test extends AbstractEngineGetPayloadTest {
  ...
  public void shouldReturnUnsupportedForkIfBlockTimestampIsBeforeSilaCancunMilestone() {
    ...
    BlockWithReceipts shanghaiBlock =
        new BlockWithReceipts(
            ...
            emptyList());
    PayloadWrapper payloadSilaShanghai =
        new PayloadWrapper(
            shanghaiPid,
            shanghaiBlock,
            Optional.empty(),
  ...
  public void shouldReturnUnsupportedForkIfBlockTimestampIsAfterSilaPragueMilestone() {
    ...
    PayloadIdentifier postSilaCancunPid =
        PayloadIdentifier.forPayloadParams(
            Hash.ZERO,
    ...
    BlockWithReceipts pragueBlock =
        new BlockWithReceipts(
            ...
            emptyList());
    PayloadWrapper payloadPostSilaCancun =
        new PayloadWrapper(
            postSilaCancunPid,
            pragueBlock,
            Optional.empty(),
    ...
    when(mergeContext.retrievePayloadById(postSilaCancunPid))
        .thenReturn(Optional.of(payloadPostSilaCancun));

    final var resp = resp(RpcMethod.ENGINE_GET_PAYLOAD_V3.getMethodName(), postSilaCancunPid);
  ...
  public void shouldReturnBlockForKnownPayloadId() {
    ...
    BlockWithReceipts postSilaCancunBlock =
        new BlockWithReceipts(
            new Block(
                cancunHeader,
                new BlockBody(
  ...
  protected long getValidPayloadTimestamp() {
    // V3 works with SilaCancun (>= 30) but must be before SilaPrague (< 50)
    return 35L;
  }
}
