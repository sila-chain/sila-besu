 ...
 */
package org.hyperledger.besu.sila.api.jsonrpc.timeout;

import static org.hyperledger.besu.sila.api.jsonrpc.RpcMethod.SIL_BLOCK_NUMBER;
import static org.hyperledger.besu.sila.api.jsonrpc.RpcMethod.SIL_GET_LOGS;
...
import org.hyperledger.besu.sila.api.handlers.TimeoutHandler;
...
public class TimeoutHandlerTest {
  ...
  public static Collection<Object[]> data() {
    return Arrays.asList(
        new Object[][] {
          {Optional.empty(), SIL_GET_LOGS, DEFAULT_OPTS.getTimeoutSeconds(), true},
          {Optional.empty(), SIL_BLOCK_NUMBER, DEFAULT_OPTS.getTimeoutSeconds(), false},
          {Optional.of(DEFAULT_OPTS), SIL_BLOCK_NUMBER, DEFAULT_OPTS.getTimeoutSeconds(), true}
        });
  }
  ...
  private static TimerSetup setupTimerCapture() {
    ...
    when(requestBody.getString("method")).thenReturn(SIL_GET_LOGS.getMethodName());
    when(ctx.data()).thenReturn(Map.of(ContextKey.REQUEST_BODY_AS_JSON_OBJECT.name(), requestBody));
    when(ctx.get(ContextKey.REQUEST_BODY_AS_JSON_OBJECT.name())).thenReturn(requestBody);
    when(ctx.vertx()).thenReturn(vertx);
