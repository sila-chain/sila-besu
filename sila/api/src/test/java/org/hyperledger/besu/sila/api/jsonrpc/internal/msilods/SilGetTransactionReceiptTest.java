...
import org.hyperledger.besu.sila.api.jsonrpc.internal.results.TransactionReceiptRootResult;
...
import org.hyperledger.besu.sila.sila-mainnet.BlockGasUsedValidator;
...
public class SilGetTransactionReceiptTest {
  ...
  private final SilGetTransactionReceipt silGetTransactionReceipt =
  ...
  private final JsonRpcRequestContext request =
      new JsonRpcRequestContext(new JsonRpcRequest("1", "sil_getTransactionReceipt", params));
  ...
  public void shouldCreateAStatusTransactionReceiptWhenStatusTypeProtocol() {
    ...
    final JsonRpcSuccessResponse response =
        (JsonRpcSuccessResponse) silGetTransactionReceipt.response(request);
  ...
  public void shouldWorkFor1559Txs() {
    ...
    final Transaction transaction1559 =
        new BlockDataGenerator().transaction(TransactionType.SIP1559);
  ...
  private void mockProtocolSpec(final BlockHeader blockHeader) {
    ...
    when(spec.getGasCalculator()).thenReturn(new SilaCancunGasCalculator());
