...
import org.hyperledger.besu.sila.api.jsonrpc.internal.results.CreateAccessListResult;
...
public class SilCreateAccessListTest {
  ...
  private static final String METHOD = "sil_createAccessList";

  private SilCreateAccessList method;
  ...
  public void setUp() {
    ...
    method = new SilCreateAccessList(blockchainQueries, transactionSimulator);
  }
  ...
  private JsonRpcRequestContext silCreateAccessListRequest(final CallParameter callParameter) {
    return new JsonRpcRequestContext(
        new JsonRpcRequest("2.0", METHOD, new Object[] {callParameter}));
  }

  private JsonRpcRequestContext silCreateAccessListRequest(
  ...
  public void shouldReturnGasEstimateWhenTransientLegacyTransactionProcessorReturnsResultSuccess() {
    final JsonRpcRequestContext request =
        silCreateAccessListRequest(legacyTransactionCallParameter(Wei.ZERO));
  ...
  public void shouldUseGasPriceParameterWhenIsPresent() {
    ...
    final JsonRpcRequestContext request =
        silCreateAccessListRequest(legacyTransactionCallParameter(gasPrice));
    mockTransactionSimulatorResult(true, false, MIN_TX_GAS_COST, pendingBlockHeader);
  ...
  public void latestBlockTagEstimateOnLatestBlock() {
    final JsonRpcRequestContext request =
        silCreateAccessListRequest(legacyTransactionCallParameter(Wei.ZERO), "latest");
  ...
  @Test
  public void shouldNotErrorWhenGasPricePresentForSip1559Transaction() {
    final Wei gasPrice = Wei.of(1000);
    final JsonRpcRequestContext request =
        silCreateAccessListRequest(sip1559TransactionCallParameter(Optional.of(gasPrice)));
    mockTransactionSimulatorResult(true, false, MIN_TX_GAS_COST, pendingBlockHeader);
  ...
  public void shouldReturnEmptyAccessListIfNoParameterAndWithoutAccessedStorage() {
    final JsonRpcResponse expectedResponse =
        new JsonRpcSuccessResponse(null, new CreateAccessListResult(List.of(), MIN_TX_GAS_COST));
    final JsonRpcRequestContext request =
        silCreateAccessListRequest(sip1559TransactionCallParameter());
    mockTransactionSimulatorResult(true, false, MIN_TX_GAS_COST, pendingBlockHeader);
  ...
  public void shouldReturnEmptyAccessListIfNoAccessedStorage() {
    ...
    final JsonRpcRequestContext request =
        silCreateAccessListRequest(sip1559TransactionCallParameter(accessListParam));
  ...
  public void shouldReturnAccessListIfParameterAndSameAccessedStorage() {
    ...
    final JsonRpcRequestContext request =
        silCreateAccessListRequest(sip1559TransactionCallParameter(expectedAccessList));
  ...
  public void shouldReturnAccessListWhenBlockTagParamIsPresent() {
    final JsonRpcRequestContext request =
        silCreateAccessListRequest(sip1559TransactionCallParameter(), "finalized");
    // Generate a random list with one access list entry
    final List<AccessListEntry> expectedAccessList = generateRandomAccessList();
  ...
  public void shouldReturnAccessListWhenBlockNumberParamIsPresent() {
    final JsonRpcRequestContext request =
        silCreateAccessListRequest(sip1559TransactionCallParameter(), "0x0");
    // Generate a random list with one access list entry
    final List<AccessListEntry> expectedAccessList = generateRandomAccessList();
  ...
  public void shouldUseTxGasLimitCapIfLessThanBlockGasLimit() {
    ...
    final JsonRpcRequestContext request =
        silCreateAccessListRequest(
            sip1559TransactionCallParameter(Optional.empty(), null, Bytes.ofUnsignedLong(1L)));
    // Generate a random list with one access list entry
    final List<AccessListEntry> expectedAccessList = generateRandomAccessList();
  ...
  private CallParameter sip1559TransactionCallParameter() {
    return sip1559TransactionCallParameter(Optional.empty(), null, Bytes.EMPTY);
  }

  private CallParameter sip1559TransactionCallParameter(final Optional<Wei> gasPrice) {
    return sip1559TransactionCallParameter(gasPrice, null, Bytes.EMPTY);
  }

  private CallParameter sip1559TransactionCallParameter(
      final List<AccessListEntry> accessListEntries) {
    return sip1559TransactionCallParameter(Optional.empty(), accessListEntries, Bytes.EMPTY);
  }
