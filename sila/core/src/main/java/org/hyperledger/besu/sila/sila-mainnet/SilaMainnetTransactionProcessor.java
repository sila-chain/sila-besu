...
public class SilaMainnetTransactionProcessor {
  ...
  public TransactionProcessingResult processTransaction(
    ...
    try {
      ...
      // SIP-8037: intrinsic gas is split into regular and state dimensions. Computed via the shared
      ...
      // SIP-8037: Validate that gas limit covers both regular AND state intrinsic gas.
      ...
      // SIP-8037: split the available gas budget into regular gas and reservoir, then bake the
      ...
      if (transaction.isContractCreation()) {
        ...
        final Code code =
            processCodeFromAccount(
                worldState, sip2930WarmAddressList, worldState.get(to), accessLocationTracker);
      ...
      // SIP-8037: regular gas consumption is bounded before execution — the initial frame is built
      ...
      // SIP-7708: Emit closure logs for accounts with remaining balance before deletion
      // Noop before SilaAmsterdam
  ...
  /**
   * SIP-8037: regular-gas / reservoir / state-gas-used split for the initial frame after applying
  ...
  private record IntrinsicStateGasCharge(long gasLeft, long reservoir, long stateGasUsed) {
    ...
    static IntrinsicStateGasCharge compute(
      ...
      if (transaction.getType().equals(TransactionType.DELEGATE_CODE)) {
        ...
        // SIP-8037: charge the full worst-case intrinsic (every authority a new account, every
