...
import static org.hyperledger.besu.datatypes.HardforkId.SilaMainnetHardforkId.EXPERIMENTAL_EIPS;
import static org.hyperledger.besu.datatypes.HardforkId.SilaMainnetHardforkId.FRONTIER;
import static org.hyperledger.besu.datatypes.HardforkId.SilaMainnetHardforkId.FUTURE_EIPS;
...
import static org.hyperledger.besu.sila.silaMainnet.requests.SilaMainnetRequestsProcessor.amsterdamRequestsProcessors;
import static org.hyperledger.besu.sila.silaMainnet.requests.SilaMainnetRequestsProcessor.pragueRequestsProcessors;
...
import org.hyperledger.besu.sila.silaMainnet.blockhash.SilaCancunPreExecutionProcessor;
...
import org.hyperledger.besu.sila.silaMainnet.requests.RequestContractAddresses;
...
public abstract class SilaMainnetProtocolSpecs {
  ...
  // A consensus bug at Sila mainnet transaction 0xcf416c53
  ...
  public static ProtocolSpecBuilder frontierDefinition(
    ...
    return new ProtocolSpecBuilder()
        ...
        .savmBuilder(SilaMainnetEVMs::frontier)
        ...
        .blockHeaderValidatorBuilder(
            (feeMarket, gasCalculator, gasLimitCalculator) ->
                SilaMainnetBlockHeaderValidator.create())
  ...
  public static ProtocolSpecBuilder homesteadDefinition(
    ...
    return frontierDefinition(
        ...
        .savmBuilder(SilaMainnetEVMs::homestead)
        .contractCreationProcessorBuilder(
            savm ->
                new ContractCreationProcessor(
  ...
  public static ProtocolSpecBuilder byzantiumDefinition(
    ...
    return spuriousDragonDefinition(
        ...
        .savmBuilder(SilaMainnetEVMs::byzantium)
  ...
  public static ProtocolSpecBuilder constantinopleDefinition(
    ...
    return byzantiumDefinition(
        ...
        .savmBuilder(SilaMainnetEVMs::constantinople)
  ...
  static ProtocolSpecBuilder londonDefinition(
    ...
    return berlinDefinition(
        ...
        .savmBuilder(
            (gasCalculator, jdCacheConfig) ->
                SilaMainnetEVMs.london(
                    gasCalculator, chainId.orElse(BigInteger.ZERO), savmConfiguration))
        .difficultyCalculator(SilaMainnetDifficultyCalculators.LONDON)
        .blockHeaderValidatorBuilder(
            (feeMarket, gasCalculator, gasLimitCalculator) ->
                SilaMainnetBlockHeaderValidator.createBaseFeeMarketValidator(
  ...
  static ProtocolSpecBuilder pragueDefinition(
    ...
    ProtocolSpecBuilder pragueSpecBuilder =
        cancunDefinition(
            ...
            // SIP-7685: requestsHash header field is mandatory from SilaPrague onwards
            .blockHeaderValidatorBuilder(
                SilaMainnetBlockHeaderValidator::requestsAwareBlockHeaderValidator)
            .hardforkId(PRAGUE);
    if (isPoAConsensus(genesisConfigOptions) && !hasSystemContractAddresses(genesisConfigOptions)) {
      LOG.warn(
  ...
  static ProtocolSpecBuilder amsterdamDefinition(
    ...
    final ProtocolSpecBuilder amsterdamSpecBuilder =
        bpo5Definition(
            ...
            .messageCallProcessorBuilder(
                (savm, precompileContractRegistry) ->
                    new MessageCallProcessor(
                        ...
                        SPURIOUS_DRAGON_FORCE_DELETE_WHEN_EMPTY_ADDRESSES,
                        SIP7708TransferLogEmitter.INSTANCE))
            // SIP-7708: TransactionProcessor configured for SilaAmsterdam with transfer log
            // emission
            .transactionProcessorBuilder(
                (gasCalculator,
                    feeMarket,
            ...
            // SIP-8037: Disable validation-time TX_MAX_GAS_LIMIT cap (enforced at runtime on
            // execution gas)
            .gasLimitCalculatorBuilder(
                (feeMarket, gasCalculator, blobSchedule) -> {
            ...
            // SilaAmsterdam: Validator uses pre-refund gas_metered = max(execution, state) from
            // processing
            .blockGasUsedValidator(BlockGasUsedValidator.AMSTERDAM)
            // SIP-7843: slotNumber is the last header field, so a header omitting it still
            // decodes cleanly - only this rule rejects it.
            .blockHeaderValidatorBuilder(
                SilaMainnetBlockHeaderValidator::slotNumberAwareBlockHeaderValidator)
            .slotNumberRequired(true)
            .hardforkId(AMSTERDAM);

    // SIP-8282 introduces the builder deposit (0x03) and builder exit (0x04) system-contract
    // requests. PoA dev/test chains without system contract addresses cannot run the system calls,
    // so they keep the no-op coordinator inherited from SilaPrague; every other chain replaces the
    // inherited SilaPrague coordinator with the SilaAmsterdam one that also collects the builder
    // requests.
    if (isPoAConsensus(genesisConfigOptions) && !hasSystemContractAddresses(genesisConfigOptions)) {
      LOG.warn(
          "Skipping system contract request processors for PoA consensus (clique/ibft/qbft) without system contract addresses.");
      ...
      try {
        ...
        LOG.warn("SilaAmsterdam definitions require system contract addresses in genesis");
        throw nsee;
      }
    }
  ...
  static ProtocolSpecBuilder futureEipsDefinition(
    ...
    return amsterdamDefinition(
            ...
            balConfiguration,
            metricsSystem)
        .precompileContractRegistryBuilder(SilaMainnetPrecompiledContractRegistries::futureEips)
  ...
  static ProtocolSpecBuilder experimentalEipsDefinition(
    ...
    return futureEipsDefinition(
        ...
        .savmBuilder(
            (gasCalculator, jdCacheConfig) ->
                SilaMainnetEVMs.experimentalEips(
                    gasCalculator, chainId.orElse(BigInteger.ZERO), savmConfiguration))
