...
public class SilaMainnetBlockBodyValidator implements BlockBodyValidator {

  private static final Logger LOG = LoggerFactory.getLogger(SilaMainnetBlockBodyValidator.class);
  ...
  public SilaMainnetBlockBodyValidator(final ProtocolSchedule protocolSchedule) {
  ...
  public boolean validateBodyLight(
    ...
    if (!validateSilHash(context, block, ommerValidationMode)) {
      return false;
    }
  ...
  private boolean validateSilHash(
      final ProtocolContext context,
      final Block block,
      final HeaderValidationMode ommerValidationMode) {
    final BlockHeader header = block.getHeader();
