 ...
 *
 * <pre>
 * (sil/68): receipt = {legacy-receipt, typed-receipt} with typed-receipt = tx-type || rlp(legacy-receipt)
 *
 * legacy-receipt = [
...
public class TransactionReceiptDecoder {
  ...
  /**
   * Creates a transaction receipt for the given RLP (Frontier/pre-SilaAmsterdam format).
   *
  ...
  public static TransactionReceipt readFrom(
    ...
    // The first byte indicates whether the receipt is typed (sil/68) or flat (sil/69).
  ...
  /**
   * Decodes a typed receipt (SIP-2718+) up to and including logs, returning the components needed
  ...
  private static TransactionReceipt decodeFlatReceipt(
      final RLPInput rlpInput, final boolean revertReasonAllowed) {
    rlpInput.enterList();
    // Flat receipts can be either legacy or sil/69 receipts.
    ...
    // The first element can be either the transaction type (sil/69 or stateRootOrStatus (sil/68
    final RLPInput firstElement = rlpInput.readAsRlp();
    // The second element can be either the state root or status (sil/68) or cumulative gas (sil/69)
  ...
  /**
   * Decodes a legacy receipt (pre-SIP-2718) up to and including logs, returning the components
