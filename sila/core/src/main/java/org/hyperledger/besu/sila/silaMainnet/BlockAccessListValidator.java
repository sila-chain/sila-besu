...
public interface BlockAccessListValidator {
   ...
   * SilaAmsterdam, where blocks must not contain a block access list.
   */
  ...
  /**
   * Full validation of an <em>imported</em> block access list (e.g. from a peer): hash, SIP-7928
   * item size, canonical ordering, and index constraints per SIP-7928.
   *
  ...
  /**
   * During block execution: SIP-7928 item-size budget only (running count of addresses plus storage
   ...
   * org.hyperledger.besu.savm.gascalculator.GasCalculator#getBlockAccessListItemCost()} so block
   * building can align with the mining {@link ProtocolSpec} instead of only the validator's
   * schedule.
   *
   * @param protocolSpecForItemCost spec whose gas calculator defines the SIP-7928 item cost
   */
  default BlockAccessListItemSizeCheck validateExecutedBlockAccessListItemSize(
   ...
   * omits {@code balHash}, returns empty. Otherwise SIP-7928 item size on the built list, then
