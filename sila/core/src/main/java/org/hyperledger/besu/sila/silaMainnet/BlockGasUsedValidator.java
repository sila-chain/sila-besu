 ...
 *
 * <p>Prior to SIP-7778: Block gas equals the last receipt's cumulativeGasUsed (post-refund).
 *
 * <p>SIP-7778 (SilaAmsterdam+): Block gas is pre-refund and may differ from receipt's
...
public interface BlockGasUsedValidator {
   ...
   *     SIP-7778, this is the pre-refund gas. Empty when doing light validation during sync.
  ...
  /**
   * Pre-SIP-7778 (Frontier through Pectra): Validates that header.gasUsed equals the last receipt's
  ...
  /**
   * SilaAmsterdam (SIP-7778 + SIP-8037): Validates header.gasUsed against cumulativeBlockGasUsed
   * when available. Header.gasUsed is pre-refund gas, while receipt.cumulativeGasUsed is
   * post-refund gas, so they differ when refunds apply.
   *
