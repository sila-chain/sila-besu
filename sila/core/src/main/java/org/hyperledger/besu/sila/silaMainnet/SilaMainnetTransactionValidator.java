 ...
 *
 * <p>The {@link SilaMainnetTransactionValidator} performs the intrinsic gas cost check on the given
 * {@link Transaction}.
 */
public class SilaMainnetTransactionValidator implements TransactionValidator {
  private static final Logger LOG = LoggerFactory.getLogger(SilaMainnetTransactionValidator.class);
  ...
  private final SilaMainnetBlobsValidator blobsValidator;

  public SilaMainnetTransactionValidator(
    ...
    this.blobsValidator =
        new SilaMainnetBlobsValidator(acceptedBlobVersions, gasLimitCalculator, gasCalculator);
  }
  ...
  public ValidationResult<TransactionInvalidReason> validate(
    ...
    if (!transactionValidationParams.isAllowExceedingGasLimit()
        // Long.MAX_VALUE is the sentinel for "no cap" (pre-SilaOsaka). Only apply unsigned
        // comparison when a real cap is in effect; unsigned would incorrectly reject
        // transactions with gas >= 2^63 on pre-SilaOsaka forks where no cap applies.
  ...
  private ValidationResult<TransactionInvalidReason> validateCostAndFee(
    ...
    if (transaction.getType().supportsBlob()) {
      ...
      if (maybeBlobFee.isEmpty()) {
        ...
        // tx.getMaxFeePerBlobGas can be empty for sil_call
    ...
    // SIP-8037: cap max(intrinsic_execution, calldata_floor) rather than tx.gas itself.
