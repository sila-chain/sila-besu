...
/*
 * Used to process transactions for sil_call and sil_estimateGas.
 *
...
public class TransactionSimulator {
  ...
  private Optional<Transaction> buildTransaction(
    ...
    if (transactionValidationParams.isPreserveCallerGasPricing()) {
      // sil_simulateV1: use caller-provided gas pricing so fees are charged from sender's balance,
      ...
      // sil_call: zero gas prices so callers don't need sufficient balance for gas.
      gasPrice = Wei.ZERO;
  ...
  private boolean shouldSetMaxFeePerGas(
    ...
    // This ensures the transaction is considered SIP-1559 only if these parameters are present
