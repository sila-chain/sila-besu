...
import org.hyperledger.besu.savm.account.MutableAccount.BalanceUnderflowException;
...
public class SilaMainnetTransactionProcessor {

  private static final Logger LOG = LoggerFactory.getLogger(SilaMainnetTransactionProcessor.class);
  ...
  private SilaMainnetTransactionProcessor(
   ...
   *     SilaMainnetTransactionValidator}
   * @return the transaction result
   * @see SilaMainnetTransactionValidator
   ...
   * @param transaction The transaction to process
   * @param miningBeneficiary The address which is to receive the transaction fee
   * @param operationTracer The tracer to record results of each SAVM operation
   * @param blockHashLookup The {@link BlockHashLookup} to use for BLOCKHASH operations
   * @return the transaction result
   */
  public TransactionProcessingResult processTransaction(
  ...
  public TransactionProcessingResult processTransaction(
    ...
    try {
      ...
      final Set<Address> sip2930WarmAddressList = new HashSet<>(Address.SIZE);
      ...
      final List<AccessListEntry> sip2930AccessListEntries =
      ...
      final Multimap<Address, Bytes32> sip2930StorageList = HashMultimap.create();
      for (final var entry : sip2930AccessListEntries) {
        final Address address = entry.address();
        sip2930WarmAddressList.add(address);
        final List<Bytes32> storageKeys = entry.storageKeys();
        sip2930StorageList.putAll(address, storageKeys);
      }
      if (warmCoinbase) {
        sip2930WarmAddressList.add(miningBeneficiary);
      }
      ...
      // SIP-2780 charges every state-dependent cost at the top frame, so the intrinsic is
      ...
      // SilaAmsterdam charges authorizations at the top frame with no refund; pre-SilaAmsterdam
      // reserves
      // the worst case in the intrinsic and refunds it here.
      long codeDelegationRefund = 0L;
      // Empty pre-SilaAmsterdam and for non-delegation transactions.
      ...
      // Null pre-SilaAmsterdam, which has no preparation charge to fail, and when there are no
      // authorizations.
      ...
      if (transaction.getType().equals(TransactionType.DELEGATE_CODE)) {
        ...
        sip2930WarmAddressList.addAll(codeDelegationResult.accessedDelegatorAddresses());
      ...
      // SIP-8037: execution gas is capped at TX_MAX_GAS_LIMIT, so anything bought above that cap
      // can
      // only ever be spent as state gas and starts in the reservoir. Pre-SilaAmsterdam the cap is
      ...
      final MessageFrame.Builder commonMessageFrameBuilder =
          MessageFrame.builder()
              ...
              .sip2930AccessListWarmStorage(sip2930StorageList);

      accessLocationTracker.ifPresent(commonMessageFrameBuilder::sip7928AccessList);
      ...
      // SIP-8037: an already-alive (e.g. pre-funded) target adds no leaf, so it owes no
      // NEW_ACCOUNT.
      boolean createTargetAlreadyAlive = false;
      if (transaction.isContractCreation()) {
        final Address contractAddress =
            Address.contractAddress(senderAddress, sender.getNonce() - 1L);
        // Nothing reads it pre-SilaAmsterdam, so skip the lookup.
        if (stateGasCalc.isActive()) {
        ...
        initialFrame =
            commonMessageFrameBuilder
                ...
                .code(code)
                .sip2930AccessListWarmAddresses(sip2930WarmAddressList)
                .build();
      } else {
      ...
      // SIP-2780: the state-dependent costs are charged against the frame, on the transaction's
      ...
      if (txSucceeded) {
        ...
        // SIP-8037: a successful creation adds the leaf it was charged for, so the charge stands.
      } else {
        // SIP-7702: delegations applied by a successful preparation outlive a dispatch failure,
        ...
        // SIP-8037: no leaf the creation or the value transfer would have added survives a failed
      ...
      // TODO SLD are the log correct following SIP-7623?
      ...
      LOG.atTrace()
          ...
          .log();
      // Calculate gas used: max of execution gas and transaction floor cost (SIP-7623)
      // For pre-SilaPrague forks, floor cost is 0, so this returns just execution gas
      // For SilaPrague+ forks with SIP-7778, this ensures block gas accounts for data floor
      // SIP-8037: Gas accounting with multidimensional gas support
      ...
      LOG.trace(
          "SIP-8037 TX_END gasUsed={} stateGasUsed={} reservoir={}",
      ...
      // SIP-158 & SIP-7928: coinbase is considered "touched" even when fees are zero.
      ...
      // SIP-7708: Emit closure (burn) logs for self-destructed accounts whose balance is burned.
      // Noop before SilaAmsterdam. SIP-8246 preserves the balance instead of burning it, so no
   ...
   * @param authorizations the SIP-7702 per-authority charges, taken as a whole
  ...
  private PrepCharges chargeTopFrame(
    ...
    // Pre-SilaAmsterdam forks pay none of these charges, but still record the recipient load below.
    ...
    if (transaction.isContractCreation()) {
      ...
      if (stateGasActive && !createTargetAlreadyAlive) {
        // SIP-8037: the created account's NEW_ACCOUNT state gas is charged only when the
      ...
      if (!outOfGas) {
        ...
        // SIP-7928: the load precedes the charge, so the recipient stays listed even when its own
  ...
  /**
   * SIP-2780: replays each authorization's top-frame access in transaction order, touching the
   * authority before charging it so an out-of-gas leaves the authorities after it out of the
   * SIP-7928 block access list. Charges never partially consume gas, so the frame stays consistent
  ...
  /**
   * Charges the SIP-2780 dispatch-entry costs on the depth-0 frame of a non-create transaction:
  ...
  private boolean chargeTransactionEntry(
    ...
    // a zero-balance precompile is not "alive" under SIP-161 either.
    ...
    // SIP-2780: the top-level access to a delegated recipient's target is warm/cold aware.
    if (recipient != null && hasCodeDelegation(recipient.getCode())) {
      ...
      // SIP-7928: the target is loaded only once its access is paid for, so an access charge that
  ...
  /**
   * Settles accounts marked for self-destruction at transaction finalization. Under SIP-8246 each
   * account is cleared (nonce reset, code and storage removed) but keeps its balance — SIP-161
   * state clearing (via {@code clearAccountsThatAreEmpty}) then removes any account left with a
   * zero balance. Pre-SIP-8246 the accounts are deleted outright.
   *
  ...
  private Code delegationTargetCode(
      ...
      final Account contract,
      final Optional<AccessLocationTracker> accessLocationTracker) {
    // Besu resolves the delegation target eagerly to build the frame, but SIP-7928 only lists it
    // once its access charge is paid, so recording is left to chargeTransactionEntry.
    // Pre-SilaAmsterdam
    ...
    // SIP-2780: pre-warming the target here would make the warm/cold-aware access charged in
    // chargeTransactionEntry always read warm. Pre-SilaAmsterdam forks have no such charge, so they
    // still warm it here.
    if (!stateGasActive) {
  ...
  public static class Builder {
    ...
    public Builder populateFrom(final SilaMainnetTransactionProcessor processor) {
    ...
    public SilaMainnetTransactionProcessor build() {
      return new SilaMainnetTransactionProcessor(
          gasCalculator,
