...
class SilaMainnetTransactionProcessorTest {
  ...
  SilaMainnetTransactionProcessor createTransactionProcessor(final boolean warmCoinbase) {
  ...
  */
  void shouldTraceEndTxOnFailingTransaction(final Exception exception) {
    ...
    try {
      ...
      // the MerkleTrieException is thrown again in SilaMainnetTransactionProcessor, we ignore it
      // here
    }
  ...
  void shouldReturnInsufficientFundsForTransferWhenValueTransferUnderflowsDuringSimulation() {
    // Reproduces the sil_estimateGas crash where the upfront gas cost is affordable (so the
