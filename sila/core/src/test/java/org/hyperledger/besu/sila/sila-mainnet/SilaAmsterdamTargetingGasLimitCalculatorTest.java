...
class SilaAmsterdamTargetingGasLimitCalculatorTest {

  // SilaAmsterdam: target=14, max=21, baseFeeUpdateFraction=11684671 (SIP-7918)
  ...
  private static final SilaAmsterdamGasCalculator gasCalculator = new SilaAmsterdamGasCalculator();
  private final SilaAmsterdamTargetingGasLimitCalculator gasLimitCalculator =
      new SilaAmsterdamTargetingGasLimitCalculator(
          0L,
  ...
  /**
   * Test SilaAmsterdam excessBlobGas calculation using the standard SilaCancun formula branch. With
   * base_fee_per_gas=7 and SilaAmsterdam's baseFeeUpdateFraction=11684671, the blob gas price is 1 at
   ...
   * [fork_SilaAmsterdam-new_blobs_1-header_blob_gas_used_1703936-blockchain_test_engine-parent_blobs_0]
  ...
  /**
   * Test SilaAmsterdam excessBlobGas calculation using the SIP-7918 reserve price branch. When
   ...
   * 131072 (at low excess where blob gas price = 1), so the SIP-7918 branch triggers. delta = 21 -
   * 14 = 7, so: excess = parentExcess + parentBlobGasUsed * 7 / 21
   */
  @ParameterizedTest(name = "{index} - parent excess {0}, used gas {1}, base fee {2}")
  @MethodSource("sip7918BranchBlobGases")
  public void shouldCalculateExcessBlobGasCorrectly_sip7918Branch(
      final long parentExcess,
      final long parentBlobGasUsed,
      final long baseFee,
      final long expected) {
  ...
  Iterable<Arguments> sip7918BranchBlobGases() {
    // parentExcess = TARGET_GAS + 1 = 1835009, just above target so not clamped to 0
    // With base_fee=17: 17*8192=139264 > 1*131072=131072 → SIP-7918 branch
