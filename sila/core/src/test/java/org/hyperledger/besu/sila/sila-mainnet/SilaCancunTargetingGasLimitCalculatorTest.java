...
class SilaCancunTargetingGasLimitCalculatorTest {
  ...
  private final SilaCancunTargetingGasLimitCalculator cancunTargetingGasLimitCalculator =
  ...
  public void shouldCalculateSilaCancunExcessBlobGasCorrectly(
      final long parentExcess, final long used, final long expected) {
    final long usedBlobGas = new SilaCancunGasCalculator().blobGasCost(used);
    assertThat(
  ...
  @Test
  void shouldUseSilaCancunCalculatorBlobGasPerBlob() {
    // should use SilaCancunGasCalculator's blob gas per blob to calculate the gas limit
    final long blobGasPerBlob = new SilaCancunGasCalculator().getBlobGasPerBlob();
  ...
  private final SilaPragueGasCalculator pragueGasCalculator = new SilaPragueGasCalculator();
  // SilaCancunTargetingGasLimitCalculator with SilaPrague numbers
  private final SilaCancunTargetingGasLimitCalculator pragueGasLimitCalculator =
      new SilaCancunTargetingGasLimitCalculator(
          0L,
          FeeMarket.cancunDefault(0L, Optional.empty()),
          pragueGasCalculator,
  ...
  public void shouldCalculateSilaPragueExcessBlobGasCorrectly(
      final long parentExcess, final long used, final long expected) {
  ...
  @Test
  void shouldCalculateCorrectlySilaPragueBlobGasPerBlob() {
    // should use SilaPragueGasCalculator's blob gas per blob to calculate the gas limit
    final long blobGasPerBlob = new SilaPragueGasCalculator().getBlobGasPerBlob();
