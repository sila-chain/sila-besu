...
import org.hyperledger.besu.savm.operation.AbstractOperation;
...
class DebugOperationTracerTest {
  ...
  private final Operation MSTORE_OPERATION =
      new AbstractOperation(0x52, "MSTORE", 2, 0, null) {
        @Override
        public OperationResult execute(final MessageFrame frame, final SAVM savm) {
          // explicitMemoryUpdate=true simulates what MSTORE does in the real SAVM
          frame.writeMemory(0L, 32, WORD_2, true);
          return new OperationResult(3L, null);
        }
      };

  private final CallOperation callOperation = new CallOperation(new SilaCancunGasCalculator());
