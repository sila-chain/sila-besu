...
import static org.hyperledger.besu.sila.silaMainnet.parallelization.ParallelBlockProcessorTestSupport.GENESIS_CONFIG;
...
public abstract class AbstractParallelBlockProcessorIntegrationTest {
   ...
   */
  static class NoBlockFallbackParallelBlockProcessor extends SilaMainnetParallelBlockProcessor {
   ...
   * sequential process then {@link org.hyperledger.besu.sila.chain.MutableBlockchain#appendBlock}).
   */
   ...
   * org.hyperledger.besu.sila.SilaMainnetBlockValidator}).
   */
   ...
   * org.hyperledger.besu.sila.silaMainnet.AbstractBlockProcessor} when at least one transaction was
   ...
   * org.hyperledger.besu.sila.silaMainnet.parallelization.SilaMainnetParallelBlockProcessor} may
   * fall back to fully sequential processing for the whole block, in which case the field stays
   * empty; we then only assert that any reported count is positive.
   */
  protected void assertParallelizationRecordedInResults(
    ...
    assertThat(sequentialResult.getNbParallelizedTransactions())
        .as("Sequential SilaMainnetBlockProcessor must not report parallelized transaction metrics")
        .isEmpty();
