...
/**
 * Acceptance test for SLOTNUM opcode (SIP-7843).
 *
 ...
 * verifying storage via {@code sil_getStorageAt} and verifying the block header via {@code
 * sil_getBlockByNumber}.
 */
public class SIP7843SlotNumOpcodeAcceptanceTest extends AcceptanceTestBase {
  ...
  void setUp() throws IOException {
    ...
    slotnumContract = Account.create(silTransactions, SLOTNUM_CONTRACT_ADDRESS);
  }
  ...
  public void slotNumReturnsCorrectSlotNumber() throws IOException {
    ...
    final String storageValue1 =
        besuNode.execute(silTransactions.getStorageAt(slotnumContract, BigInteger.ZERO));
  ...
  private void verifyBlockHeaderSlotNumber(
    ...
    final Optional<BlockWithSlotNumber> maybeBlock =
        besuNode.execute(silTransactions.getBlockWithSlotNumber(blockNumber));
