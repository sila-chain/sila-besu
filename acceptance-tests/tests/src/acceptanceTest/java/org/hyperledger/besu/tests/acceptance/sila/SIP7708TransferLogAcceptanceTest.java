 ...
 *
 * <p>SIP-7708 specifies that all nonzero SIL value transfers emit a LOG3-equivalent log with:
 *
 * <ul>
...
public class SIP7708TransferLogAcceptanceTest extends AcceptanceTestBase {
  ...
  private static final String SIP7708_SYSTEM_ADDRESS = "0xfffffffffffffffffffffffffffffffffffffffe";
  ...
  public void shouldEmitTransferLogForSimpleSilTransfer() throws IOException {
    ...
    final String txHash =
        besuNode.execute(silTransactions.sendRawTransaction(tx.encoded().toHexString()));
    ...
    WaitUtils.waitFor(
        60,
        () -> {
          maybeReceiptHolder.set(besuNode.execute(silTransactions.getTransactionReceipt(txHash)));
    ...
    // Verify the log is from the SIP-7708 system address
    assertThat(transferLog.getAddress()).isEqualToIgnoringCase(SIP7708_SYSTEM_ADDRESS);
  ...
  public void shouldNotEmitTransferLogForZeroValueTransfer() throws IOException {
    ...
    // Zero-value transfers should NOT emit an SIP-7708 log
    assertThat(receipt.getLogs()).isEmpty();
  }

  /**
   * Test that internal CALL with value emits multiple SIP-7708 transfer logs.
   *
   * <p>This test sends SIL to a contract (0x9999) that forwards all received SIL to an address
  ...
  public void shouldEmitMultipleTransferLogsForContractCallWithValue() throws IOException {
    ...
    assertThat(log1.getAddress()).isEqualToIgnoringCase(SIP7708_SYSTEM_ADDRESS);
    ...
    assertThat(log2.getAddress()).isEqualToIgnoringCase(SIP7708_SYSTEM_ADDRESS);
  ...
  /**
   * Test that SELFDESTRUCT to a DIFFERENT address emits an SIP-7708 Transfer log (LOG3).
   *
   * <p>This test calls a contract (0x7708) that self-destructs and sends its balance to an address
   * provided in calldata. Since the beneficiary is different from the contract itself, an SIP-7708
  ...
  public void shouldEmitTransferLogForSelfDestructToDifferentAddress() throws IOException {
    ...
    // Contract has 1 SIL balance from genesis
  ...
  /**
   * Test that SELFDESTRUCT to SELF for a pre-existing contract emits NO log under SIP-6780.
   *
   * <p>With SIP-6780 (post-SilaCancun), SELFDESTRUCT only destroys contracts created in the same
  ...
  public void shouldNotEmitLogForPreExistingContractSelfDestructToSelf() throws IOException {
    ...
    // Under SIP-6780, pre-existing contracts selfdestructing to self is a no-op.
    // No destruction happens, no value moves, so no SIP-7708 log should be emitted.
    assertThat(receipt.getLogs())
        .as("Pre-existing contract SELFDESTRUCT to self should NOT emit any logs under SIP-6780")
        .isEmpty();
  }

  /**
   * Test that pre-existing contracts selfdestructing to self emit NO logs under SIP-6780.
   *
   ...
   *
   * <p>Under SIP-6780 (post-SilaCancun), SELFDESTRUCT only destroys contracts created in the same
   ...
   *
   * <p>Per SIP-7708, since no actual destruction or value transfer occurs, no logs should be
   * emitted.
   */
  @Test
  public void shouldNotEmitLogsForPreExistingContractsSelfDestructingToSelf() throws IOException {
    ...
    // Under SIP-6780, pre-existing contracts selfdestructing to self is a no-op.
    // No destruction happens, no value moves, so no SIP-7708 logs should be emitted.
    assertThat(receipt.getLogs())
        .as("Pre-existing contracts SELFDESTRUCT to self should NOT emit any logs under SIP-6780")
        .isEmpty();
  }

  /**
   * Test that reverted transactions do NOT emit SIP-7708 transfer logs.
   *
   * <p>This test sends SIL to a contract (0x666) that immediately reverts. Even though value was
   * sent, no SIP-7708 transfer log should be emitted because the transaction reverted.
   */
  @Test
  public void shouldNotEmitTransferLogForRevertedTransaction() throws IOException {
    ...
    // Reverted transactions should NOT emit any SIP-7708 logs
    assertThat(receipt.getLogs()).isEmpty();
  }
   ...
   *   <li>Sends its entire balance (1 SIL) to the child
