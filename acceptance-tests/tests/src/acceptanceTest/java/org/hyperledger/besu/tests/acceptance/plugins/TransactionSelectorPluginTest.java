...
public class TransactionSelectorPluginTest extends AcceptanceTestBase {
  ...
  public void transactionIsMined() {
    ...
    node.verify(sil.expectSuccessfulTransactionReceipt(txHash.getBytes().toHexString()));
  }
  ...
  public void badMultipleOfValueTransactionIsNotSelectedByPlugin(final long amount) {
    ...
    node.verify(
        sil.expectSuccessfulTransactionReceipt(anotherSenderGoodHash.getBytes().toHexString()));
    node.verify(sil.expectNoTransactionReceipt(badHash.getBytes().toHexString()));
    // good tx from the same sender is not mined due to the nonce gap
    node.verify(sil.expectNoTransactionReceipt(sameSenderGoodHash.getBytes().toHexString()));
  }
  ...
  public void unhandledExceptionInPluginSelectorIsManaged() {
    ...
    Awaitility.await()
        .until(
            () -> {
              final var nextBlock = node.execute(silTransactions.blockNumber()).add(BigInteger.ONE);
    ...
    node.verify(
        sil.expectSuccessfulTransactionReceipt(dummyTransferTxHash.getBytes().toHexString()));
    final var dummyTransferReceipt =
        node.execute(
            silTransactions.getTransactionReceipt(dummyTransferTxHash.getBytes().toHexString()));
    ...
    final var preSendBlock = node.execute(silTransactions.blockNumber());
    final var throwingTxHash = node.execute(throwingTransferTx);
    final var goodTxHash = node.execute(goodTransferTx);

    node.verify(sil.expectSuccessfulTransactionReceipt(goodTxHash.getBytes().toHexString()));

    final var goodReceipt =
        node.execute(silTransactions.getTransactionReceipt(goodTxHash.getBytes().toHexString()));
    assertThat(goodReceipt.get().getBlockNumber()).isEqualTo(preSendBlock.add(BigInteger.ONE));

    node.verify(sil.expectNoTransactionReceipt(throwingTxHash.getBytes().toHexString()));
  }
  ...
  public void pluginsSeeContentOfPublicTransactionPool() throws IOException {
    ...
    final var receipt =
        node.execute(silTransactions.getTransactionReceipt(txHash.getBytes().toHexString()));
