...
public class SilEstimateGasAcceptanceTest extends AcceptanceTestBase {
  ...
  public void estimateGasWithDelegateCall() {
    ...
    for (var test : testCase) {
      ...
      // Subtracting 10% should make the sil_call fail
      ...
      // Sanity check our estimate is close by sending sil_call with less than that gas estimate
      var silCallTooLow =
          node.execute(
              new SilCallTransaction(testDepth.getContractAddress(), functionCall, gasTooLow));

      assertThat(silCallTooLow.isReverted()).isEqualTo(true);

      // Sanity check our estimate is close with sil_sendRawTransaction
