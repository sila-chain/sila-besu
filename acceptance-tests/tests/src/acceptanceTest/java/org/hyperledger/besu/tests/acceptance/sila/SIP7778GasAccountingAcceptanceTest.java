...
/**
 * Acceptance tests for SIP-7778: Block gas accounting without refunds.
 *
 * <p>SIP-7778 changes block gas accounting to use pre-refund gas for block gas limit enforcement,
 * preventing circumvention of the block gas limit via refunds.
 *
 * <p>Key changes in SIP-7778:
 *
 * <ul>
 ...
 * </ul>
 */
public class SIP7778GasAccountingAcceptanceTest extends AcceptanceTestBase {
  ...
  /** Tests that simple SIL transfers work correctly with SIP-7778 block gas accounting. */
  @Test
  ...
  public void shouldProcessTransactionWithRefundSuccessfully() throws IOException {
    ...
    // The SSTORE clear generates a 4800 gas refund (SSTORE_CLEARS_SCHEDULE in post-SIP-3529)
  ...
  /**
   * Tests that multiple transactions in a block work correctly with SIP-7778 gas accounting. Block
  ...
  public void shouldProcessMultipleTransactionsInBlock() throws IOException {
    ...
    final String txHash1 =
        besuNode.execute(silTransactions.sendRawTransaction(tx1.encoded().toHexString()));
    final String txHash2 =
        besuNode.execute(silTransactions.sendRawTransaction(tx2.encoded().toHexString()));
    testHelper.buildNewBlock();
    ...
    WaitUtils.waitFor(
        60,
        () -> {
          receipt1Holder.set(besuNode.execute(silTransactions.getTransactionReceipt(txHash1)));
          receipt2Holder.set(besuNode.execute(silTransactions.getTransactionReceipt(txHash2)));
