...
public class CodeDelegationTransactionAcceptanceTest extends AcceptanceTestBase {
  ...
  private SilaPragueAcceptanceTestHelper testHelper;
  ...
  void setUp() throws IOException {
    ...
    testHelper = new SilaPragueAcceptanceTestHelper(besuNode, silTransactions);
  }
   ...
   * 90000 SIL. The authorizer creates an authorization for a contract that send all its SIL to any
   * given address. The transaction sponsor sponsors the 7702 transaction and sends all the SIL from
    ...
    WaitUtils.waitFor(
        ...
        () -> {
          maybeTransactionReceiptHolder.set(
              besuNode.execute(silTransactions.getTransactionReceipt(txHash)));
  ...
  /**
   * The authorizer creates an authorization for a contract that sends all its SIL to any given
   ...
   * the transaction sponsor's * balance should be 180000 SIL minus the transaction costs.
   */
  @Test
  public void shouldCheckNonceAfterNonceIncreaseOfSender() throws IOException {
    ...
    WaitUtils.waitFor(
        ...
        () -> {
          authorizerBalanceAfterFirstTxHolder.set(
              besuNode.execute(silTransactions.getBalance(authorizer)));
    ...
    // The other accounts balance should be the initial 9000 SIL balance from the authorizer minus
    ...
    WaitUtils.waitFor(
        ...
        () -> {
          otherAccountBalanceAfterSecondTxHolder.set(
              besuNode.execute(silTransactions.getBalance(otherAccount)));
  ...
  /**
   * SIP-7702 code delegation should be persisted even if the transaction that contains the
  ...
  public void shouldPersistCodeDelegationAfterRevert() throws IOException {
    ...
    WaitUtils.waitFor(
        30,
        () -> {
          authorizerCodeHolder.set(besuNode.execute(silTransactions.getCode(authorizer)));
