...
public class Account {
  ...
  private final SilTransactions sil;
  ...
  private Account(final SilTransactions sil, final String name, final KeyPair keyPair) {
    this(
        sil,
        name,
        Address.extract(
  ...
  public static Account create(final SilTransactions sil, final Address address) {
    return new Account(sil, address.toString(), address, Optional.empty());
  }

  public static Account create(final SilTransactions sil, final String name) {
    return new Account(sil, name, SIGNATURE_ALGORITHM.generateKeyPair());
  }

  public static Account fromPrivateKey(
      final SilTransactions sil, final String name, final String privateKey) {
    return new Account(
        sil,
        name,
  ...
  public Condition balanceEquals(final Amount expectedBalance) {
    return new ExpectAccountBalance(
        sil, this, expectedBalance.getValue(), expectedBalance.getUnit());
  }
  ...
  public Condition balanceAtBlockEquals(final Amount expectedBalance, final BigInteger block) {
    return new ExpectAccountBalanceAtBlock(
        sil, this, block, expectedBalance.getValue(), expectedBalance.getUnit());
  }
