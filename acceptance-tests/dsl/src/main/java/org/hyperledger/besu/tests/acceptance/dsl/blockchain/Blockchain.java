...
public class Blockchain {
  ...
  public Blockchain(final SilTransactions sil) {
    this.sil = sil;
  }

  public Condition blockNumberMustBeLatest(final Node node) {
    return new ExpectMinimumBlockNumber(sil, currentHeight(node));
  }

  public Condition beneficiaryEquals(final BesuNode node) {
    return new ExpectBeneficiary(sil, node);
  }

  public Condition minimumHeight(final long blockNumber) {
    return new ExpectBlockNumberAbove(sil, BigInteger.valueOf(blockNumber));
  }

  public Condition minimumHeight(final long blockNumber, final int timeout) {
    return new ExpectBlockNumberAbove(sil, BigInteger.valueOf(blockNumber), timeout);
  }

  public Condition reachesHeight(final BesuNode node, final int blocksAheadOfLatest) {
    return new ExpectBlockNumberAbove(sil, futureHeight(node, blocksAheadOfLatest));
  }

  public Condition reachesHeight(
      final BesuNode node, final int blocksAheadOfLatest, final int timeout) {
    return new ExpectBlockNumberAbove(sil, futureHeight(node, blocksAheadOfLatest), timeout);
  }

  public Condition currentHeight(final long blockNumber) {
    return new ExpectBlockNumber(sil, BigInteger.valueOf(blockNumber));
  }
  ...
  private BigInteger currentHeight(final Node node) {
    return node.execute(sil.blockNumber());
  }
}
