  ...
  public T execute(final NodeRequests node) {
    try {
      if (args != null && args.length != 0) {
        ...
        parameterObjects.addAll(Arrays.asList(node.sil(), BENEFACTOR_ONE, gasPrice, gasLimit));
        ...
        final Object invoked =
            method.invoke(METHOD_IS_STATIC, node.sil(), BENEFACTOR_ONE, gasPrice, gasLimit);
