...
import io.promsileus.metrics.expositionformats.PromsileusTextFormatWriter;
...
public class MetricsHttpServiceTest {
  ...
  private PromsileusMetricsSystem metricsSystem;
  ...
  private void initServerAndClient(
    ...
    metricsSystem = (PromsileusMetricsSystem) MetricsSystemFactory.create(metricsConfiguration);
  ...
  private MetricsHttpService createMetricsHttpService(
      final MetricsConfiguration config, final PromsileusMetricsSystem metricsSystem) {
  ...
  public void acceptHeaderIgnored() throws Exception {
    ...
    try (final Response resp = client.newCall(metricsRequest).execute()) {
      ...
      // We should have JVM metrics already loaded, verify a simple key.
      assertThat(props).containsKey("jvm_threads_deadlocked");
      assertThat(resp.header("Content-Type")).contains(PromsileusTextFormatWriter.CONTENT_TYPE);
    }
  }
