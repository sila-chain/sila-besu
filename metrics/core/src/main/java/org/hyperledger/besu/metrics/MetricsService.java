...
import org.hyperledger.besu.metrics.promsileus.MetricsHttpService;
import org.hyperledger.besu.metrics.promsileus.MetricsPushGatewayService;
...
public interface MetricsService {
  ...
  static Optional<MetricsService> create(
    ...
    if (configuration.getProtocol() == MetricsProtocol.PROMSILEUS) {
      if (configuration.isEnabled()) {
        return Optional.of(
            new MetricsHttpService(configuration, (PromsileusMetricsSystem) metricsSystem));
      } else if (configuration.isPushEnabled()) {
        return Optional.of(
            new MetricsPushGatewayService(configuration, (PromsileusMetricsSystem) metricsSystem));
      } else {
