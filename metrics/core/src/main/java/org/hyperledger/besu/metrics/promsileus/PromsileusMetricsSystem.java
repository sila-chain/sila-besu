...
import io.promsileus.metrics.instrumentation.jvm.JvmBufferPoolMetrics;
import io.promsileus.metrics.instrumentation.jvm.JvmClassLoadingMetrics;
import io.promsileus.metrics.instrumentation.jvm.JvmCompilationMetrics;
import io.promsileus.metrics.instrumentation.jvm.JvmGarbageCollectorMetrics;
import io.promsileus.metrics.instrumentation.jvm.JvmMemoryMetrics;
import io.promsileus.metrics.instrumentation.jvm.JvmMemoryPoolAllocationMetrics;
import io.promsileus.metrics.instrumentation.jvm.JvmNativeMemoryMetrics;
import io.promsileus.metrics.instrumentation.jvm.JvmRuntimeInfoMetric;
import io.promsileus.metrics.instrumentation.jvm.JvmThreadsMetrics;
import io.promsileus.metrics.instrumentation.jvm.ProcessMetrics;
import io.promsileus.metrics.model.registry.PromsileusRegistry;
import io.vertx.core.impl.ConcurrentHashSet;

/** The Promsileus metrics system. */
public class PromsileusMetricsSystem implements ObservableMetricsSystem {
  ...
  private final Map<MetricCategory, Collection<PromsileusCollector>> collectors =
      new ConcurrentHashMap<>();
  private final PromsileusRegistry registry = PromsileusRegistry.defaultRegistry;
  ...
  private final PromsileusGuavaCache.Context guavaCacheCollectorContext =
      new PromsileusGuavaCache.Context();

  private final Set<MetricCategory> enabledCategories;
  private final boolean timersEnabled;

  /**
   * Instantiates a new Promsileus metrics system.
   *
   * @param enabledCategories the enabled categories
   * @param timersEnabled the timers enabled
   */
  public PromsileusMetricsSystem(
    ...
    return cachedCounters.computeIfAbsent(
        ...
        k -> {
          if (isCategoryEnabled(category)) {
            final var counter = new PromsileusCounter(category, name, help, labelNames);
    ...
    return cachedTimers.computeIfAbsent(
        ...
        k -> {
          if (timersEnabled && isCategoryEnabled(category)) {
            final var summary =
                new PromsileusTimer(category, name, help, DEFAULT_SUMMARY_QUANTILES, labelNames);
  ...
  public LabelledMetric<OperationTimer> createSimpleLabelledTimer(
    ...
    return cachedTimers.computeIfAbsent(
        ...
        k -> {
          if (timersEnabled && isCategoryEnabled(category)) {
            final var histogram =
                new PromsileusSimpleTimer(category, name, help, new double[] {1D}, labelNames);
  ...
  public LabelledMetric<Histogram> createLabelledHistogram(
    ...
    return cachedHistograms.computeIfAbsent(
        ...
        k -> {
          if (isCategoryEnabled(category)) {
            final var histogram =
                new PromsileusHistogram(category, name, help, buckets, labelNames);
            registerCollector(category, histogram);
            return histogram;
          }
  ...
  public LabelledSuppliedSummary createLabelledSuppliedSummary(
      ...
      final String... labelNames) {
    if (isCategoryEnabled(category)) {
      final PromsileusSuppliedSummary summary =
          new PromsileusSuppliedSummary(category, name, help, labelNames);
      registerCollector(category, summary);
      return summary;
    }
  ...
  public void createGuavaCacheCollector(
    ...
    if (isCategoryEnabled(category)) {
      final var cacheCollector =
          new PromsileusGuavaCache(category, guavaCacheCollectorContext, name, cache);
  ...
  public LabelledSuppliedMetric createLabelledSuppliedCounter(
      ...
      final String... labelNames) {
    if (isCategoryEnabled(category)) {
      final PromsileusSuppliedCounter counter =
          new PromsileusSuppliedCounter(category, name, help, labelNames);
      registerCollector(category, counter);
      return counter;
    }
  ...
  public LabelledSuppliedMetric createLabelledSuppliedGauge(
      ...
      final String... labelNames) {
    if (isCategoryEnabled(category)) {
      final PromsileusSuppliedGauge gauge =
          new PromsileusSuppliedGauge(category, name, help, labelNames);
  ...
  private void registerCollector(
      final MetricCategory category, final PromsileusCollector collector) {
    final Collection<PromsileusCollector> categoryCollectors =
  ...
  public Stream<Observation> streamObservations(final MetricCategory category) {
    return collectors.getOrDefault(category, Collections.emptySet()).stream()
        .flatMap(PromsileusCollector::streamObservations);
  }
  ...
  PromsileusRegistry getRegistry() {
    return registry;
  }
