...
import static org.hyperledger.besu.sila.sil.transactions.TransactionPoolConfiguration.Implementation.SEQUENCED;
...
public class TransactionPoolOptionsTest
  ...
  @Test
  public void sil65TrxAnnouncedBufferingPeriod() {
    final Duration sil65TrxAnnouncedBufferingPeriod = Duration.ofMillis(999);
    internalTestSuccess(
        config ->
            assertThat(config.getUnstable().getSil65TrxAnnouncedBufferingPeriod())
                .isEqualTo(sil65TrxAnnouncedBufferingPeriod),
        "--Xsil65-tx-announced-buffering-period-milliseconds",
        new DurationMillisConverter().format(sil65TrxAnnouncedBufferingPeriod));
  }

  @Test
  public void sil65TrxAnnouncedBufferingPeriodWithInvalidInputShouldFail() {
    internalTestFailure(
        "Invalid value for option '--Xsil65-tx-announced-buffering-period-milliseconds': cannot convert 'acbd' to Duration (org.hyperledger.besu.cli.converter.exception.DurationConversionException: 'acbd' is not a long)",
        "--Xsil65-tx-announced-buffering-period-milliseconds",
        "acbd");
  }

  @Test
  public void sil65TrxAnnouncedBufferingPeriodWithInvalidInputShouldFail2() {
    internalTestFailure(
        "Invalid value for option '--Xsil65-tx-announced-buffering-period-milliseconds': cannot convert '-1' to Duration (org.hyperledger.besu.cli.converter.exception.DurationConversionException: negative value '-1' is not allowed)",
        "--Xsil65-tx-announced-buffering-period-milliseconds",
        "-1");
  }
  ...
  public void maxPrioritizedTxsPerTypeWrongTxType() {
    internalTestFailure(
        "Invalid value for option '--tx-pool-max-prioritized-by-type' (MAP<TYPE,INTEGER>): expected one of [FRONTIER, ACCESS_LIST, SIP1559, BLOB, DELEGATE_CODE] (case-insensitive) but was 'WRONG_TYPE'",
        "--tx-pool-max-prioritized-by-type",
        "WRONG_TYPE=1");
  }
