...
import static org.hyperledger.besu.sila.core.BlockHeader.GENESIS_BLOCK_NUMBER;
...
public class ConsensusScheduleBesuControllerBuilder extends BesuControllerBuilder {
        ...
        .forEach(
            entry -> {
              ...
              for (int i = 0; i < subProtocols.size(); i++) {
                ...
                // Only add if not already present (avoid duplicates like SilProtocol)
  ...
  public BesuControllerBuilder silProtocolConfiguration(
    ...
    besuControllerBuilderSchedule
        .values()
        .forEach(b -> b.silProtocolConfiguration(silProtocolConfiguration));
    return super.silProtocolConfiguration(silProtocolConfiguration);
  }
  ...
  public BesuControllerBuilder savmConfiguration(final SavmConfiguration savmConfiguration) {
    besuControllerBuilderSchedule.values().forEach(b -> b.savmConfiguration(savmConfiguration));
    return super.savmConfiguration(savmConfiguration);
  }
